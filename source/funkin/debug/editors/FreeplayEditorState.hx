...
{
	public static var songInfo:FreeplayListData;
	...
	{
		for (i in 0...songInfo.songs.length)
		{
			var s = songInfo.songs[i];
			addSong(s.name, s.group ?? i, s.icon ?? 'bf');
			var ci = Std.parseInt(s.color ?? '0xFFFFD900');
			coolColors.push(ci != null ? ci : 0xFFFFD900);
		}
	}
	...
	function loadSongsData():Void
	{
		...
		if (mods.ModManager.isActive())
		{
			...
			{
				final entries:Array<funkin.data.FreeplayList.FreeplaySongEntry> = [];
				for (modWeek in mods.compat.ModCompatLayer.getModSongsInfo())
				{
					...
					for (j in 0...ws.length)
						entries.push({ name: ws[j], icon: (si!=null&&j<si.length)?si[j]:'bf', group: 0 });
				}
				songInfo = { songs: entries };
				return;
			}
		}
		#end
		songInfo = FreeplayList.load();
	}
	...
	function removeSong(songIndex:Int):Void
	{
		...
		// Remove from freeplayListData
		songInfo = FreeplayList.remove(songInfo, songToRemove.songName);
	...
	{
		// Rebuild freeplayListData from current songs order
		songInfo.songs = [];
		for (s in songs)
		{
			var colorInt = (s.week < coolColors.length) ? coolColors[s.week] : s.color;
			var entry:funkin.data.FreeplayList.FreeplaySongEntry = {
				name:  s.songName,
				icon:  s.songCharacter,
				color: '0x' + StringTools.hex(colorInt, 8),
				group: s.week
			};
			songInfo.songs.push(entry);
		}
	}

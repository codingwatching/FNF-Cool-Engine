...
//  TYPEDEFS
// ─────────────────────────────────────────────────────────────────────────────
typedef SPortDef = { n:String, t:String };
typedef SNodeDef = { label:String, color:Int, cat:String,
                     inputs:Array<SPortDef>, outputs:Array<SPortDef> };
typedef SNodeData= { id:Int, type:String, wx:Float, wy:Float,
                     params:Map<String,Dynamic> };
typedef SConnData= { fromId:Int, fromPort:Int, toId:Int, toPort:Int };
typedef SNViewIt = { s:FlxSprite, ox:Float, oy:Float };
typedef SNViewTx = { t:FlxText,   ox:Float, oy:Float };
typedef SNodeView= { sprs:Array<SNViewIt>, txts:Array<SNViewTx>, nodeH:Int };
...
class ShaderEditorSubState extends flixel.FlxSubState
{
    ...
    static function __init__():Void
    {
        DEFS = [
        // ── Output ────────────────────────────────────────────────────────────
        "output"    => { label:"Output",        color:0xFF1B5E20, cat:"Output",
            inputs: [{n:"Color",t:"vec4"},{n:"Alpha",t:"float"}],
            outputs:[] },
        // ── Texture & UV ─────────────────────────────────────────────────────
        "texture"   => { label:"Texture Sample", color:0xFF0D47A1, cat:"Texture",
            inputs: [{n:"UV",t:"vec2"}],
            outputs:[{n:"RGBA",t:"vec4"},{n:"R",t:"float"},{n:"G",t:"float"},
                     {n:"B",t:"float"},{n:"A",t:"float"}] },
        "uv"        => { label:"UV Coords",      color:0xFF33691E, cat:"Coords",
            inputs: [],
            outputs:[{n:"UV",t:"vec2"},{n:"U",t:"float"},{n:"V",t:"float"}] },
        "screen_uv" => { label:"Screen UV",      color:0xFF33691E, cat:"Coords",
            inputs: [],
            outputs:[{n:"UV",t:"vec2"}] },
        // ── Constants ─────────────────────────────────────────────────────────
        "color"     => { label:"Color",          color:0xFF4A148C, cat:"Value",
            inputs: [],
            outputs:[{n:"RGBA",t:"vec4"}] },
        "float_val" => { label:"Float",          color:0xFF263238, cat:"Value",
            inputs: [],
            outputs:[{n:"Value",t:"float"}] },
        "time"      => { label:"Time",           color:0xFF00695C, cat:"Value",
            inputs: [],
            outputs:[{n:"Time",t:"float"}] },
        // ── Math ──────────────────────────────────────────────────────────────
        "add"       => { label:"Add  A+B",       color:0xFF3E2723, cat:"Math",
            inputs: [{n:"A",t:"any"},{n:"B",t:"any"}],
            outputs:[{n:"Result",t:"any"}] },
        "multiply"  => { label:"Multiply  A×B",  color:0xFF3E2723, cat:"Math",
            inputs: [{n:"A",t:"any"},{n:"B",t:"any"}],
            outputs:[{n:"Result",t:"any"}] },
        "mix"       => { label:"Mix / Lerp",     color:0xFF3E2723, cat:"Math",
            inputs: [{n:"A",t:"any"},{n:"B",t:"any"},{n:"T",t:"float"}],
            outputs:[{n:"Result",t:"any"}] },
        "sine"      => { label:"Sine",           color:0xFF880E4F, cat:"Math",
            inputs: [{n:"X",t:"float"}],
            outputs:[{n:"Sin",t:"float"}] },
        "pow_node"  => { label:"Power",          color:0xFF880E4F, cat:"Math",
            inputs: [{n:"Base",t:"float"},{n:"Exp",t:"float"}],
            outputs:[{n:"Result",t:"float"}] },
        "clamp_node"=> { label:"Clamp",          color:0xFF880E4F, cat:"Math",
            inputs: [{n:"Value",t:"float"},{n:"Min",t:"float"},{n:"Max",t:"float"}],
            outputs:[{n:"Result",t:"float"}] },
        // ── Color ops ─────────────────────────────────────────────────────────
        "grayscale" => { label:"Grayscale",      color:0xFF37474F, cat:"Color",
            inputs: [{n:"Color",t:"vec4"}],
            outputs:[{n:"Gray",t:"float"},{n:"RGBA",t:"vec4"}] },
        "invert"    => { label:"Invert",         color:0xFF37474F, cat:"Color",
            inputs: [{n:"Color",t:"vec4"}],
            outputs:[{n:"Color",t:"vec4"}] },
        "split"     => { label:"Split RGBA",     color:0xFF1B5E20, cat:"Color",
            inputs: [{n:"Color",t:"vec4"}],
            outputs:[{n:"R",t:"float"},{n:"G",t:"float"},{n:"B",t:"float"},{n:"A",t:"float"}] },
        "combine"   => { label:"Combine RGBA",   color:0xFF1B5E20, cat:"Color",
            inputs: [{n:"R",t:"float"},{n:"G",t:"float"},{n:"B",t:"float"},{n:"A",t:"float"}],
            outputs:[{n:"Color",t:"vec4"}] },
        // ── Effects ───────────────────────────────────────────────────────────
        "fresnel"   => { label:"Fresnel",        color:0xFF0D47A1, cat:"FX",
            inputs: [{n:"Power",t:"float"}],
            outputs:[{n:"Value",t:"float"}] },
        "wave_uv"   => { label:"Wave UV",        color:0xFF4A148C, cat:"FX",
            inputs: [{n:"UV",t:"vec2"},{n:"Speed",t:"float"},{n:"Amp",t:"float"}],
            outputs:[{n:"UV",t:"vec2"}] },
        "chromatic" => { label:"Chromatic Ab.",  color:0xFF4A148C, cat:"FX",
            inputs: [{n:"UV",t:"vec2"},{n:"Shift",t:"float"}],
            outputs:[{n:"Color",t:"vec4"}] },
        // ── Raw GLSL (imported from file) ─────────────────────────────────────
        "raw_glsl"  => { label:"Raw GLSL",       color:0xFF37474F, cat:"Raw",
            inputs: [],
    ...
    var _addMenuBtns   : Array<{id:String,x:Float,y:Float,w:Float,h:Float}> = [];

    // HUD buttons
    var _hudBtns : Array<{id:String,x:Float,y:Float,w:Float,h:Float}> = [];
    ...
    var _browserBtns     : Array<{id:String,x:Float,y:Float,w:Float,h:Float}> = [];
    ...
    // ── Constructor ───────────────────────────────────────────────────────────
    public function new(name:String, ?frag:String, ?vert:String,
        ?spr:FlxSprite, ?cam:FlxCamera, onSave:String->String->Void)
    {
        super();
        _name   = name;
        _onSave = onSave;
    ...
    function _addNode(type:String, wx:Float, wy:Float):Int
    {
        ...
        switch (type) {
            case "color":
                params.set("r", 1.0); params.set("g", 0.5);
                params.set("b", 0.0); params.set("a", 1.0);
            case "float_val": params.set("value", 0.5);
            case "pow_node":  params.set("exp", 2.0);
            case "clamp_node":params.set("min", 0.0); params.set("max", 1.0);
        }

        _nodes.push({ id:id, type:type, wx:wx, wy:wy, params:params });
        _buildNodeView(id);
        return id;
    }
    ...
    {
        if (_getNode(id) != null && _getNode(id).type == "output") return; // can't delete Output
        _conns = _conns.filter(c -> c.fromId != id && c.toId != id);
        _nodes = _nodes.filter(n -> n.id != id);
        _deleteView(id);
        if (_selectedId == id) { _selectedId = -1; _updateProps(); }
        _updatePreview();
    }
    ...
    function _connectPorts(fId:Int, fP:Int, tId:Int, tP:Int):Void
    {
        ...
        _conns.push({ fromId:fId, fromPort:fP, toId:tId, toPort:tP });
    }

    function _getNode(id:Int):Null<SNodeData>
    {
        for (n in _nodes) if (n.id == id) return n;
        return null;
    }
    ...
    {
        var node = _getNode(id); if (node == null) return;
        var def  = DEFS.get(node.type); if (def == null) return;
        ...
        // Shadow
        var shadow = new FlxSprite(); shadow.makeGraphic(NODE_W + 4, nodeH + 4, 0x66000000);
        addS(shadow, 3, 3);

        // Body
        var body = new FlxSprite(); body.makeGraphic(NODE_W, nodeH, C_NODEBG);
        addS(body, 0, 0);

        // Title bar
        var titleBg = new FlxSprite(); titleBg.makeGraphic(NODE_W, NODE_TITL, def.color);
        addS(titleBg, 0, 0);
        ...
        for (i in 0...def.inputs.length) {
            ...
            var dot = new FlxSprite(); dot.makeGraphic(Std.int(PORT_R * 2), Std.int(PORT_R * 2), 0xFF000000 | col);
        ...
        // Selection border
        var border = new FlxSprite(); border.makeGraphic(NODE_W, nodeH, 0x00000000, true);
    ...
    {
        var node = _getNode(id); if (node == null) return;
        var view = _views.get(id); if (view == null) return;
        for (it in view.sprs) { it.s.x = node.wx + it.ox; it.s.y = node.wy + it.oy; }
        for (it in view.txts) { it.t.x = node.wx + it.ox; it.t.y = node.wy + it.oy; }
    }
    ...
    {
        var view = _views.get(id); if (view == null) return;
        for (it in view.sprs) { _nodeGroup.remove(it.s, true); it.s.destroy(); }
        for (it in view.txts) { remove(it.t, true); it.t.destroy(); }
        _views.remove(id);
    }
    ...
    {
        var node = _getNode(nodeId); if (node == null) return null;
    ...
    function _hitPort(wx:Float, wy:Float):{nodeId:Int, portIdx:Int, isOut:Bool}
    {
        ...
        for (node in _nodes) {
            var def = DEFS.get(node.type); if (def == null) continue;
            // Inputs (left edge)
            for (i in 0...def.inputs.length) {
                var py = node.wy + NODE_TITL + i * PORT_H + PORT_H / 2;
                var dx = wx - node.wx; var dy = wy - py;
                if (dx*dx + dy*dy < R2) return { nodeId:node.id, portIdx:i, isOut:false };
            }
            // Outputs (right edge)
            for (i in 0...def.outputs.length) {
                var py = node.wy + NODE_TITL + i * PORT_H + PORT_H / 2;
                var dx = wx - (node.wx + NODE_W); var dy = wy - py;
                if (dx*dx + dy*dy < R2) return { nodeId:node.id, portIdx:i, isOut:true };
            }
        }
        return null;
    }
    ...
    function _hitNode(wx:Float, wy:Float):Int
    {
        ...
        while (i >= 0) {
            ...
            if (view != null) {
                if (wx >= node.wx && wx <= node.wx + NODE_W &&
                    wy >= node.wy && wy <= node.wy + view.nodeH)
                    return node.id;
            }
            i--;
        }
    ...
    function _redrawWires():Void
    {
        ...
        for (c in _conns) {
            ...
            if (from == null || to == null) { from?.put(); to?.put(); continue; }

            var srcNode = _getNode(c.fromId);
            var def     = srcNode != null ? DEFS.get(srcNode.type) : null;
            var col     = (def != null && c.fromPort < def.outputs.length)
                          ? _pcol(def.outputs[c.fromPort].t) : PC_ANY;
        ...
        if (_pending != null) {
            var fp = _pending.isOut
                ? _portWPos(_pending.nodeId, _pending.portIdx, true)
                : _portWPos(_pending.nodeId, _pending.portIdx, false);
            if (fp != null) {
                g.lineStyle(1.5, C_ACCENT, 0.75);
                if (_pending.isOut) _bezier(g, fp.x, fp.y, _mouseWX, _mouseWY);
                else                _bezier(g, _mouseWX, _mouseWY, fp.x, fp.y);
                fp.put();
            }
        }
    }
    ...
    function _redrawGrid():Void
    {
        ...
        while (x <= ex) {
            ...
            while (y <= ey) {
                ...
                if (isMajX && isMajY) {
                    g.beginFill(0x3A3A60, 1); g.drawRect(x - 1.5, y - 1.5, 3, 3); g.endFill();
                } else {
                    g.beginFill(0x222238, 1); g.drawRect(x - 0.5, y - 0.5, 1, 1); g.endFill();
                }
                y += step;
            }
            x += step;
    ...
    function _generateGLSL():String
    {
        ...
        for (n in _nodes) if (n.type == "output") { outNode = n; break; }
        if (outNode == null) return "#pragma header\nvoid main(){gl_FragColor=vec4(1.0);}";

        // Topological sort (DFS from output backwards)
        var sorted:Array<Int> = [];
        var vis:Map<Int,Bool> = new Map();
        function visit(id:Int):Void {
            if (vis.get(id) == true) return;
            vis.set(id, true);
            for (c in _conns) if (c.toId == id) visit(c.fromId);
            sorted.push(id);
        }
        visit(outNode.id);

        var needTime = false;
        for (n in _nodes) if (n.type == "time" || n.type == "wave_uv" || n.type == "chromatic") { needTime = true; break; }

        var sb = new StringBuf();
        sb.add("#pragma header\n");
        if (needTime) sb.add("uniform float time;\n");
        sb.add("\nvoid main() {\n");

        for (nodeId in sorted) {
            var node = _getNode(nodeId);
            if (node == null || node.type == "output") continue;
            var snip = _nodeSnippet(node);
            if (snip != null) sb.add(snip);
        }
    ...
    function _nodeSnippet(node:SNodeData):Null<String>
    {
        ...
        return switch (node.type) {
            ...
            case "color":
                var r = _pf(node.params.get("r")); var g = _pf(node.params.get("g"));
                var b = _pf(node.params.get("b")); var a = _pf(node.params.get("a"));
            ...
            case "add":
                var a = _inVar(node.id, 0, "0.0"); var b = _inVar(node.id, 1, "0.0");
                '  vec4 $vn = vec4($a) + vec4($b);\n';
            case "multiply":
                var a = _inVar(node.id, 0, "1.0"); var b = _inVar(node.id, 1, "1.0");
                '  vec4 $vn = vec4($a) * vec4($b);\n';
            case "mix":
                var a = _inVar(node.id, 0, "0.0"); var b = _inVar(node.id, 1, "1.0");
            ...
            case "clamp_node":
                ...
                var mn  = _pf(node.params.get("min")); var mx = _pf(node.params.get("max"));
            ...
            case "split":
                ...
                '  vec4 $vn = $c;\n';
            case "combine":
                var r = _inVar(node.id,0,"0.0"); var g = _inVar(node.id,1,"0.0");
                var b = _inVar(node.id,2,"0.0"); var a = _inVar(node.id,3,"1.0");
                '  vec4 $vn = vec4($r,$g,$b,$a);\n';
            case "fresnel":
    ...
    {
        var node = _getNode(nodeId); if (node == null) return "0.0";
        var vn   = 'v_$nodeId';
    ...
    function _extractGLSLBody(code:String):String
    {
        ...
        if (mainIdx < 0) return "  // (no main found in raw shader)\n";
        var open = code.indexOf("{", mainIdx);
        var close = code.lastIndexOf("}");
        if (open < 0 || close < open) return "  // (malformed raw shader)\n";
    ...
    function _updateProps():Void
    {
        ...
        for (i in 1..._propLines.length) if (_propLines[i] != null) _propLines[i].text = "";
        ...
        var node = _getNode(_selectedId); if (node == null) return;
        var def  = DEFS.get(node.type);  if (def  == null) return;
        ...
        // Ports list
        for (p in def.inputs)  lines.push({ txt: '← ${p.n}  [${p.t}]', col: 0xFFAABBFF });
        for (p in def.outputs) lines.push({ txt: '→ ${p.n}  [${p.t}]', col: 0xFFAAFFBB });
        ...
        for (i in 0...lines.length) {
            var idx = i + 1;
            if (idx >= _propLines.length || _propLines[idx] == null) break;
    ...
    function _showAddMenu(scrX:Float, scrY:Float, wX:Float, wY:Float):Void
    {
        ...
        for (type in DEFS.keys()) {
            ...
            if (!bycat.exists(cat)) { bycat.set(cat, []); cats.push(cat); }
            bycat.get(cat).push(type);
        }
        cats.sort((a,b) -> a < b ? -1 : 1);
        for (cat in cats) bycat.get(cat).sort((a,b) -> a < b ? -1 : 1);

        var mW   = 180;
        var mH   = 22;
        for (cat in cats) mH += 14 + bycat.get(cat).length * 18;

        var mX = scrX; if (mX + mW > FlxG.width - PANEL_W) mX = scrX - mW;
        var mY = scrY; if (mY + mH > FlxG.height)           mY = scrY - mH;

        // Background
        var bg = new FlxSprite(mX, mY); bg.makeGraphic(mW, mH, 0xFF0E0E1E);
        _hud(bg); _addMenuSprArr.push(bg);
        var bdr = new FlxSprite(mX, mY); bdr.makeGraphic(mW, mH, 0x00000000, true);
        _nodeBorder(bdr, mW, mH, C_ACCENT); _hud(bdr); _addMenuSprArr.push(bdr);

        var py = mY + 5;
        var hdr = _hudTxt("ADD NODE", mX + 6, py, mW - 8, 12, C_ACCENT, LEFT);
        _addMenuTxtArr.push(hdr); py += 16;

        for (cat in cats) {
            var catLbl = _hudTxt(cat.toUpperCase(), mX + 6, py, mW - 8, 10, C_GRAY, LEFT);
            _addMenuTxtArr.push(catLbl); py += 12;

            for (type in bycat.get(cat)) {
                var def = DEFS.get(type);
                var itBg = new FlxSprite(mX + 2, py); itBg.makeGraphic(mW - 4, 17, 0xFF151525);
                _hud(itBg); _addMenuSprArr.push(itBg);

                // Color strip on left
                var strip = new FlxSprite(mX + 2, py); strip.makeGraphic(3, 17, def.color);
                _hud(strip); _addMenuSprArr.push(strip);

                var itLbl = _hudTxt(def.label, mX + 10, py + 3, mW - 14, 11, C_WHITE, LEFT);
                _addMenuTxtArr.push(itLbl);

                final t = type;
                _addMenuBtns.push({ id:"addnode:" + t, x:mX + 2, y:py, w:mW - 4, h:17 });
                py += 18;
            }
            py += 2;
        }
    ...
    {
        _addMenuOpen = false;
        for (s in _addMenuSprArr) { remove(s, true); s.destroy(); }
        for (t in _addMenuTxtArr) { remove(t, true); t.destroy(); }
        _addMenuSprArr = []; _addMenuTxtArr = []; _addMenuBtns = [];
    }
    ...
    function _saveShader():Void
    {
        ...
        var name = _name.trim();
        if (name.endsWith('.frag')) name = name.substr(0, name.length - 5);
        if (name == "") name = "node_shader";

        #if sys
        var dir = mods.ModManager.isActive()
            ? '${mods.ModManager.modRoot()}/shaders' : 'assets/shaders';
        try {
            if (!FileSystem.exists(dir)) FileSystem.createDirectory(dir);
            File.saveContent('$dir/$name.frag', glsl);
        } catch (ex:Dynamic) { _setStatus('Write error: $ex'); return; }
        shaders.ShaderManager.shaders.remove(name);
        shaders.ShaderManager.scanShaders();
        if (_onSave != null) _onSave(name, glsl);
    ...
    function _showShaderBrowser():Void
    {
        ...
        shaders.ShaderManager.scanShaders();
        _browserList = shaders.ShaderManager.getAvailableShaders();
        ...
        // Dim overlay
        var ov = new FlxSprite(0, 0); ov.makeGraphic(FlxG.width, FlxG.height, 0xBB000000);
        _hud(ov); _browserSprs.push(ov);

        // Panel bg + border
        var bg = new FlxSprite(bX, bY); bg.makeGraphic(bW, bH, 0xFF0F0F1E);
        _hud(bg); _browserSprs.push(bg);
        var bdr = new FlxSprite(bX, bY); bdr.makeGraphic(bW, bH, 0x00000000, true);
        _nodeBorder(bdr, bW, bH, C_ACCENT); _hud(bdr); _browserSprs.push(bdr);

        // Title bar
        var titleH = 34;
        var tbg = new FlxSprite(bX, bY); tbg.makeGraphic(bW, titleH, C_TITBAR);
        _hud(tbg); _browserSprs.push(tbg);
        var tline = new FlxSprite(bX, bY + titleH - 2); tline.makeGraphic(bW, 2, C_ACCENT);
        tline.alpha = 0.5; _hud(tline); _browserSprs.push(tline);

        var hdr = _hudTxt("  📁  LOAD SHADER", bX + 8, bY + 9, bW - 60, 12, C_ACCENT, LEFT);
        _browserTxts.push(hdr);

        // Close button
        var closeBg = new FlxSprite(bX + bW - 30, bY + 4); closeBg.makeGraphic(26, 26, 0xFF2A0808);
        _hud(closeBg); _browserSprs.push(closeBg);
        var closeTxt = _hudTxt("✕", bX + bW - 30, bY + 8, 26, 11, C_RED, CENTER);
        _browserTxts.push(closeTxt);
        _browserBtns.push({ id:"browser_close", x:bX + bW - 30, y:bY + 4, w:26, h:26 });

        // Search hint
        var hint = _hudTxt(
            _browserList.length == 0
                ? "No shaders found in assets/shaders/"
                : '${_browserList.length} shader(s) found — click to preview · Ctrl+click to import as node',
            bX + 8, bY + titleH + 6, bW - 16, 11, C_GRAY, LEFT);
        ...
        for (i in 0...maxItems) {
            ...
            if (idx >= _browserList.length) break;
            ...
            var rowBg = new FlxSprite(bX + 4, iy); rowBg.makeGraphic(bW - 8, itemH - 2, rowCol);
            _hud(rowBg); _browserSprs.push(rowBg);

            // Left accent bar (color by loaded state)
            var barCol = isLoaded ? C_GREEN : C_ACCENT;
            var bar = new FlxSprite(bX + 4, iy); bar.makeGraphic(3, itemH - 2, barCol);
            _hud(bar); _browserSprs.push(bar);
            ...
            // Preview button
            var prevBtn = new FlxSprite(bX + bW - 70, iy + 3); prevBtn.makeGraphic(62, itemH - 7, 0xFF082028);
            _hud(prevBtn); _browserSprs.push(prevBtn);
            var prevT = _hudTxt("▶ Preview", bX + bW - 70, iy + 6, 62, 11, C_ACCENT, CENTER);
            _browserTxts.push(prevT);

            final n = name;
            _browserBtns.push({ id:'browser_preview:$n', x:bX + 4,     y:iy, w:bW - 80, h:itemH - 2 });
            _browserBtns.push({ id:'browser_load:$n',    x:bX + bW - 70, y:iy, w:62,      h:itemH - 2 });
        }
        ...
        if (_browserScrollY > 0) {
            ...
            _browserBtns.push({ id:"browser_up", x:bX + 4, y:bY + bH - 22, w:80, h:18 });
        }
        if (_browserScrollY + maxItems < _browserList.length) {
            ...
            _browserBtns.push({ id:"browser_down", x:bX + bW - 90, y:bY + bH - 22, w:80, h:18 });
        }

        // "Back to graph mode" if in raw mode
        if (_rawFragCode != null) {
            var clearBtn = new FlxSprite(bX + 8, bY + bH - 20); clearBtn.makeGraphic(bW - 16, 16, 0xFF2A1010);
            _hud(clearBtn); _browserSprs.push(clearBtn);
            ...
            _browserBtns.push({ id:"browser_clear_raw", x:bX + 8, y:bY + bH - 20, w:bW - 16, h:16 });
        }
    }
    ...
    {
        _browserOpen = false;
        for (s in _browserSprs) { remove(s, true); s.destroy(); }
        for (t in _browserTxts) { remove(t, true); t.destroy(); }
        _browserSprs = []; _browserTxts = []; _browserBtns = [];
    }
    ...
    function _previewShaderFromDisk(name:String):Void
    {
        #if sys
        shaders.ShaderManager.scanShaders();
        var paths = shaders.ShaderManager.shaderPaths;
        if (!paths.exists(name)) { _setStatus('Shader "$name" not found on disk'); return; }
        try {
    ...
    function _loadShaderAsNode(name:String):Void
    {
        ...
        shaders.ShaderManager.scanShaders();
        var paths = shaders.ShaderManager.shaderPaths;
        if (!paths.exists(name)) { _setStatus('Shader "$name" not found'); return; }
        try {
            var code = sys.io.File.getContent(paths.get(name));
    ...
    override function update(elapsed:Float):Void
    {
        ...
        _mouseWX = wm.x; _mouseWY = wm.y; wm.put();
        ...
        if (FlxG.mouse.wheel != 0 && !_inHUD(sx, sy)) {
            if (_browserOpen) {
                ...
                _hideBrowser(); _showShaderBrowser();
            } else {
        ...
        if (FlxG.mouse.justPressed) {
            ...
            // Right panel
            if (_inHUD(sx, sy)) { if (_addMenuOpen) _hideAddMenu(); return; }
            ...
            if (_addMenuOpen) {
                ...
                for (b in _addMenuBtns)
                    if (sx >= b.x && sx <= b.x + b.w && sy >= b.y && sy <= b.y + b.h)
                        { _onHudBtn(b.id); handled = true; break; }
                _hideAddMenu();
                if (handled) return;
            }
            ...
            if (pH != null) {
                if (_pending != null) {
                    ...
                    if (_pending.isOut != pH.isOut) {
                        var fId:Int; var fP:Int; var tId:Int; var tP:Int;
                        if (_pending.isOut) {
                            fId = _pending.nodeId; fP = _pending.portIdx;
                            tId = pH.nodeId;       tP = pH.portIdx;
                        } else {
                            fId = pH.nodeId;       fP = pH.portIdx;
                            tId = _pending.nodeId; tP = _pending.portIdx;
                        }
            ...
            if (_pending != null) { _pending = null; return; }
            ...
            if (_selectedId != prevSel) {
                if (prevSel >= 0) _refreshBorder(prevSel);
                if (_selectedId >= 0) _refreshBorder(_selectedId);
                _updateProps();
            }
        }

        // Release drag
        if (FlxG.mouse.released && _draggingId >= 0) _draggingId = -1;
    ...
    function _onHudBtn(id:String):Void
    {
        ...
        if (id.startsWith("browser_preview:")) {
            ...
            _hideBrowser(); _showShaderBrowser(); // refresh with loaded indicator
            return;
        }
        ...
        switch (id) {
            ...
            case "browser_clear_raw":
                _rawFragCode = null; _rawShaderName = "";
                _updatePreview(); _hideBrowser();
                _setStatus('⟲ Returned to Node Graph mode');
            case "add_node": _showAddMenu(195, TOOLBAR_H + 4, _mouseWX, _mouseWY);
            case "load_shader": _showShaderBrowser();
            case "apply":    _updatePreview();
            case "save":     _saveShader();
            case "close":    close();
        }
    }
    ...
    function _onKeyDown(e:KeyboardEvent):Void
    {
        switch (e.keyCode) {
            case Keyboard.ESCAPE:
                if (_browserOpen)       { _hideBrowser(); return; }
                if (_addMenuOpen)       { _hideAddMenu(); return; }
                else if (_pending != null) _pending = null;
                else close();
            case Keyboard.S: if (e.ctrlKey) _saveShader();
            case Keyboard.F5: _updatePreview();
            case Keyboard.A: if (!e.ctrlKey && !_addMenuOpen && !_browserOpen)
                _showAddMenu(210, TOOLBAR_H + 10, _mouseWX, _mouseWY);
            case Keyboard.L: if (!_browserOpen) _showShaderBrowser();
            case Keyboard.DELETE, Keyboard.BACKSPACE:
                if (!_browserOpen && _selectedId > 0) _removeNode(_selectedId);
            case Keyboard.F:
                if (_selectedId >= 0) {
    ...
    function _hud<T:FlxSprite>(s:T):T {
        s.scrollFactor.set(); s.cameras = [_hudCam]; add(s); return s;
    }
    ...
    function _hudTxt(txt:String, x:Float, y:Float, w:Float, sz:Int, col:Int, align:flixel.text.FlxTextAlign):FlxText {
        ...
        t.scrollFactor.set(); t.cameras = [_hudCam]; add(t); return t;
    }

    function _mkHudBtn(bx:Float, by:Float, bw:Float, bh:Float, lbl:String, col:Int, id:String):Void {
        var r = (col >> 16) & 0xFF; var g = (col >> 8) & 0xFF; var b = col & 0xFF;
        var dim:Int = (0xFF << 24) | (Std.int(r * 0.2) << 16) | (Std.int(g * 0.2) << 8) | Std.int(b * 0.2);
        var bg = new FlxSprite(bx, by); bg.makeGraphic(Std.int(bw), Std.int(bh), dim);
        _hud(bg);
        // Left accent strip
        var strip = new FlxSprite(bx, by); strip.makeGraphic(2, Std.int(bh), col);
        _hud(strip);
        var t = _hudTxt(lbl, bx + 5, by + (bh - 9) / 2, bw - 7, 12, col, LEFT);
        _hudBtns.push({ id:id, x:bx, y:by, w:bw, h:bh });
    }
    ...
    override function close():Void
    {
        ...
        _hideAddMenu();
        // Remove stage shapes
        if (_gridShape != null && FlxG.stage.contains(_gridShape)) FlxG.stage.removeChild(_gridShape);
        if (_wireShape != null && FlxG.stage.contains(_wireShape)) FlxG.stage.removeChild(_wireShape);
        if (_nodeCam != null) { FlxG.cameras.remove(_nodeCam, true); _nodeCam = null; }
        if (_hudCam  != null) { FlxG.cameras.remove(_hudCam,  true); _hudCam  = null; }
        super.close();
    }
}

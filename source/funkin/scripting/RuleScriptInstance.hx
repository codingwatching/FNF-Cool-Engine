...
class RuleScriptInstance implements IScript
{
	...
	public function loadFile(?path:String):RuleScriptInstance
	{
		...
		if (p == null) { _error('loadFile(): no path provided'); return this; }
		filePath = p;
		if (!FileSystem.exists(p)) { _error('File not found: $p'); return this; }
		try { return loadString(File.getContent(p)); }
		catch (e:Dynamic) { _error('Error reading $p: $e'); }
		#end
		return this;
	}
	...
	public function call(fn:String, ?args:Array<Dynamic>):Dynamic
	{
		...
		if (Lua.type(_lua, -1) != 6) { Lua.pop(_lua, 1); return null; } // LUA_TFUNCTION = 6

		for (a in args) _pushOOP(_lua, a);
	...
	{
		#if (LUA_ALLOWED && linc_luajit)
		if (!active || _lua == null) return false;
		Lua.getglobal(_lua, name);
	...
	{
		#if (LUA_ALLOWED && sys)
		if (_lua == null || filePath == null) return false;
		if (!FileSystem.exists(filePath)) { trace('[RuleScript] hotReload: not found "$filePath"'); return false; }
		try
		{
		...
		catch (e:Dynamic) { trace('[RuleScript] hotReload error "$id": $e'); return false; }
		#else
		return false;
		#end
	}
	...
	function _preprocessImports(src:String):String
	{
		...
		return importReg.map(src, function(r:EReg):String
		{
			...
			{
				for (pkg in ['flixel', 'flixel.util', 'flixel.tweens', 'flixel.text',
				             'funkin', 'funkin.gameplay', 'funkin.data', 'funkin.menus'])
				{
					resolved = Type.resolveClass('$pkg.$fullName');
					if (resolved == null) resolved = Type.resolveEnum('$pkg.$fullName');
	...
	{
		// game   — PlayState instance (or nil if not in gameplay)
		_pushOOP(_lua, funkin.gameplay.PlayState.instance);
		Lua.setglobal(_lua, 'game');

		// character shortcuts
		final ps = funkin.gameplay.PlayState.instance;
		_pushOOP(_lua, ps != null ? try ps.boyfriend catch(_) null : null); Lua.setglobal(_lua, 'bf');
		_pushOOP(_lua, ps != null ? try ps.dad       catch(_) null : null); Lua.setglobal(_lua, 'dad');
		_pushOOP(_lua, ps != null ? try ps.gf        catch(_) null : null); Lua.setglobal(_lua, 'gf');

		// stage
		_pushOOP(_lua, ps != null ? try ps.currentStage catch(_) null : null); Lua.setglobal(_lua, 'stage');

		// FlxG — always available
		_pushOOP(_lua, flixel.FlxG); Lua.setglobal(_lua, 'FlxG');

		// Conductor
		_pushOOP(_lua, funkin.data.Conductor); Lua.setglobal(_lua, 'Conductor');

		// Paths
		_pushOOP(_lua, Paths); Lua.setglobal(_lua, 'Paths');

		// NoteSkinSystem — acceso directo a skins, splashes y shaders RGB
		_pushOOP(_lua, funkin.gameplay.notes.NoteSkinSystem); Lua.setglobal(_lua, 'NoteSkinSystem');

		// NoteRGBShader — clase del shader para instanciación manual si se necesita
		_pushOOP(_lua, funkin.shaders.NoteRGBShader); Lua.setglobal(_lua, 'NoteRGBShader');
	}
	...
	function _registerBridge():Void
	{
		...
		r('__haxe_index', function(l) {
			...
			try { val = Reflect.getProperty(obj, k); } catch(_) {}
			if (val == null) try { val = Reflect.field(obj, k); } catch(_) {}
			...
			if (val != null && Reflect.isFunction(val))
			{
				...
				Lua.pushnumber(l, h);
				Lua.pushstring(l, k);
				if (Lua.pcall(l, 2, 1, 0) != 0) { Lua.pop(l, 1); Lua.pushnil(l); }
				return 1;
			}
		...
		r('__haxe_newindex', function(l) {
			...
			try       { Reflect.setProperty(obj, k, v); }
			catch (e) { try Reflect.setField(obj, k, v) catch(e2) trace('[RuleScript] set $k: $e2'); }
			return 0;
		});
		...
		r('__haxe_call_method', function(l) {
			...
			{
				var m:Dynamic = null;
				try { m = Reflect.getProperty(obj, name); } catch(_) {}
				if (m == null) try { m = Reflect.field(obj, name); } catch(_) {}
				if (m == null || !Reflect.isFunction(m)) { Lua.pushnil(l); return 1; }
				_pushOOP(l, Reflect.callMethod(obj, m, args));
			}
			catch (e:Dynamic) { trace('[RuleScript] call $name: $e'); Lua.pushnil(l); }
			return 1;
		});
		...
		r('__haxe_import', function(l) {
			...
			if (_clsReg.exists(name)) { Lua.pushnumber(l, _clsReg.get(name)); return 1; }

			var resolved:Dynamic = Type.resolveClass(name);
			if (resolved == null) resolved = Type.resolveEnum(name);
			if (resolved == null)
			{
				// Try common short names
				for (pkg in ['flixel', 'flixel.util', 'flixel.tweens', 'flixel.text',
				             'flixel.group', 'flixel.sound', 'flixel.math',
				             'funkin.gameplay', 'funkin.menus', 'funkin.states',
				             'funkin.data', 'funkin.scripting', 'funkin.gameplay.notes',
				             'funkin.gameplay.objects', 'funkin.gameplay.objects.characters',
				             'funkin.gameplay.objects.stages',
				             'animationdata', 'openfl.display', 'openfl.media'])
				{
		...
		r('__haxe_new_instance', function(l) {
			...
			if (cls == null) { Lua.pushnil(l); return 1; }
			try
			{
		...
		r('__haxe_static_get', function(l) {
			...
			if (cls == null) { Lua.pushnil(l); return 1; }
			var val:Dynamic = null;
			try { val = Reflect.field(cls, k); }   catch(_) {}
			if (val == null) try { val = Reflect.getProperty(cls, k); } catch(_) {}
		...
		r('__haxe_static_set', function(l) {
			...
			try Reflect.setField(cls, k, v) catch(_) {};
			return 0;
		});
		...
		r('import', function(l) {
			...
			if (Lua.pcall(l, 1, 1, 0) != 0) { Lua.pop(l, 1); Lua.pushnil(l); return 1; }
			final h = Std.int(Lua.tonumber(l, -1));
			Lua.pop(l, 1);

			if (h == 0) { Lua.pushnil(l); return 1; }
		...
		r('require', function(l) {
			...
			final sh = Std.int(Lua.tonumber(l, -1)); Lua.pop(l, 1);

			final script:RuleScriptInstance = cast _timerScripts.get(sh);
			if (script == null) { Lua.pushnil(l); return 1; }
			...
			if (!mod.active) { Lua.pushnil(l); return 1; }
				...
				while (Lua.next(mod._lua, -2) != 0)
				{
					if (Lua.type(mod._lua, -2) == 4) // string key
					{
						...
						if (!k.startsWith('_') && !k.startsWith('__'))
						{
							...
							if (v != null) Reflect.setField(modTable, k, v);
						}
					}
					Lua.pop(mod._lua, 1);
				}
		...
		final _patchMethod = function(l:Dynamic, mode:Int):Int
		{
			final h    = _handle(l, 1);
			final name = Lua.tostring(l, 2);
			if (Lua.type(l, 3) != 6) { Lua.settop(l, 0); return 0; }

			final obj = _reg.get(h);
			if (obj == null) { Lua.settop(l, 0); return 0; }

			// Capture original (may be null for replace — that's fine)
			var original:Dynamic = null;
			try { original = Reflect.getProperty(obj, name); } catch(_) {}
			if (original == null) try { original = Reflect.field(obj, name); } catch(_) {}
			...
			final callLua = function(args:Array<Dynamic>):Dynamic {
				...
				if (Lua.type(luaRef, -1) != 6) { Lua.pop(luaRef, 1); return null; }
				for (a in args) _pushOOP(luaRef, a);
				if (Lua.pcall(luaRef, args.length, 1, 0) != 0) {
					final e = Lua.tostring(luaRef, -1); Lua.pop(luaRef, 1);
			...
			final callOriginal = function(args:Array<Dynamic>):Dynamic {
				if (capturedOriginal == null) return null;
				try return Reflect.callMethod(capturedObj, capturedOriginal, args)
				catch(_) return null;
			};
			...
			final wrapper:Dynamic = switch (mode) {
				case 0: // before — lua first, then original (unless lua returns false)
					function(args:haxe.Rest<Dynamic>):Dynamic {
						...
						final r = callLua(a);
						if (r == false) return null;  // Lua returned false → cancel original
			...
			try { Reflect.setField(obj, name, wrapper); }
			catch(e) { trace('[RuleScript] patch($name): $e'); }
		...
		r('overrideMethod', function(l) {
			...
			final wrapper:Dynamic = function(args:haxe.Rest<Dynamic>):Dynamic
			{
				...
				final a = args.toArray();
				for (arg in a) _pushOOP(luaRef, arg);
			...
			try { Reflect.setField(capturedObj, name, wrapper); }
			catch(e) { trace('[RuleScript] overrideMethod: could not patch $name — $e'); }
		...
		r('switchState', function(l) {
			...
			if (cls == null) { trace('[RuleScript] switchState: not found — $name'); return 0; }
	...
	function _registerLegacy():Void
	{
		...
		r('gamepadPressed',     _fnPadP);   r('gamepadJustPressed',_fnPadJP);
		r('mouseRightPressed',  _fnMouseRP); r('mouseRightJustPressed',_fnMouseRJP);
		...
		r('getSongName',  _fnSongName); r('getSongArtist', _fnSongArtist);
		r('isStoryMode',  _fnIsStory);  r('getDifficulty', _fnGetDiff);
		r('getAccuracy',  _fnGetAcc);   r('getSicks',      _fnGetSicks);
		r('getGoods',     _fnGetGoods); r('getBads',       _fnGetBads);
		r('getShits',     _fnGetShits); r('setSicks',      _fnSetSicks);
		r('setScrollSpeed',_fnSetScroll); r('getScrollSpeed',_fnGetScroll);
	...
	static function _oopStdlib():String return '
-- ════════════════════════════════════════════════════════════════════════════
...
';
	...
	static function _legacyStdlib():String return '
-- ════════════════════════════════════════════════════════════════════════════
...
';
	...
	static function _pushOOP(l:Dynamic, v:Dynamic):Void
	{
		...
		{
			// Register and wrap as OOP proxy
			final h = _regCtr++;
			_reg.set(h, v);
	...
	{
		if (fn == null) { Lua.pushnil(l); return; }
		final h = _regCtr++;
		_reg.set(h, fn);
	...
	{
		final t = Lua.type(l, idx);
		if (t == 3) return Std.int(Lua.tonumber(l, idx));
		if (t == 5)
		{
			Lua.getfield(l, idx, '__hid');
			final h = Std.int(Lua.tonumber(l, -1));
	...
	static function _pushLuaTable(l:Dynamic, obj:Dynamic):Void _pushLuaAnon(l, obj);

	// ── Helpers ───────────────────────────────────────────────────────────────

	static inline function _ps():Dynamic       return funkin.gameplay.PlayState.instance;
	static inline function _spr(l:Dynamic, idx:Int = 1):Dynamic
		return _reg.get(_handle(l, idx) != 0 ? _handle(l, idx) : _tags.get(Lua.tostring(l, idx) ?? '') ?? -1);
	...
	static function _resolvePath(path:String, root:Dynamic):Dynamic
	{
		...
		var o = root;
		for (p in path.split('.')) { try o = Reflect.getProperty(o, p) catch(_) return null; }
		return o;
	}
	...
	static function _applyPath(path:String, v:Dynamic, root:Dynamic):Void
	{
		...
		var o = root;
		for (i in 0...parts.length - 1) try o = Reflect.getProperty(o, parts[i]) catch(_) return;
		try       { Reflect.setProperty(o, parts[parts.length - 1], v); }
		catch (_) { try Reflect.setField(o, parts[parts.length - 1], v) catch(_) {}; }
	}
	...
	static function _luaTableToOpts(l:Dynamic, idx:Int):Dynamic
	{
		...
		while (Lua.next(l, idx) != 0)
		{
			if (Lua.type(l, -2) == 4)
			{
				...
				if (vt == 1)      Reflect.setField(opts, key, Lua.toboolean(l, -1));
				else if (vt == 3) Reflect.setField(opts, key, Lua.tonumber(l, -1));
				else if (vt == 4) Reflect.setField(opts, key, Lua.tostring(l, -1));
			}
			Lua.pop(l, 1);
		}
		return opts;
	...
	static function _resolveModulePath(path:String, scriptPath:Null<String>):Null<String>
	{
		...
		if (FileSystem.exists(path + '.lua')) return path + '.lua';
		if (scriptPath != null)
		{
			final sep = scriptPath.indexOf('/') >= 0 ? '/' : '\\';
			final dir = scriptPath.substring(0, scriptPath.lastIndexOf(sep) + 1);
			if (FileSystem.exists(dir + path))          return dir + path;
			if (FileSystem.exists(dir + path + '.lua')) return dir + path + '.lua';
		}
		if (mods.ModManager.isActive())
		{
			final mod = '${mods.ModManager.modRoot()}/$path';
			if (FileSystem.exists(mod))          return mod;
			if (FileSystem.exists(mod + '.lua')) return mod + '.lua';
		}
		#end
		return null;
	}
	...
	static function _defaultFactories():Map<String, Array<Dynamic>->Dynamic>
	{
		return [
			...
			'FlxText'        => a -> new flixel.text.FlxText(
				a.length > 0 ? (a[0]:Float) : 0, a.length > 1 ? (a[1]:Float) : 0,
				a.length > 2 ? Std.int(a[2]) : 0, a.length > 3 ? Std.string(a[3]) : '',
				a.length > 4 ? Std.int(a[4]) : 16),
	...
	static function _fnNew(l:Dynamic):Int
	{
		...
		try {
			final obj = factory != null ? factory(args) : Type.createInstance(
				Type.resolveClass(cls) ?? Type.resolveClass('funkin.gameplay.$cls')
				?? Type.resolveClass('funkin.menus.$cls') ?? Type.resolveClass('funkin.states.$cls')
				?? Type.resolveClass('funkin.gameplay.notes.$cls'), args);
			if (obj == null) { Lua.pushnil(l); return 1; }
			_pushOOP(l, obj);
		} catch (e:Dynamic) { trace('[RuleScript] newObject($cls): $e'); Lua.pushnil(l); }
		return 1;
	}

	static function _fnGetProp(l:Dynamic):Int
	{
		final h = _handle(l, 1); final f = Lua.tostring(l, 2); Lua.settop(l, 0);
		_pushOOP(l, _resolvePath(f, _reg.get(h))); return 1;
	}
	static function _fnSetProp(l:Dynamic):Int
	{
		final h = _handle(l, 1); final f = Lua.tostring(l, 2); final v = _readOOP(l, 3);
		Lua.settop(l, 0); _applyPath(f, v, _reg.get(h)); return 0;
	}
	static function _fnCallMethod(l:Dynamic):Int
	{
		final h  = _handle(l, 1); final m = Lua.tostring(l, 2);
		final na = Lua.gettop(l) - 2;
		final ar = [for (i in 0...na) _readOOP(l, i + 3)]; Lua.settop(l, 0);
		final ob = _reg.get(h); if (ob == null) { Lua.pushnil(l); return 1; }
		try {
			var fn:Dynamic = null;
			try fn = Reflect.getProperty(ob, m) catch(_) {};
			if (fn == null) fn = Reflect.field(ob, m);
			_pushOOP(l, Reflect.callMethod(ob, fn, ar));
		} catch (e:Dynamic) { trace('[RuleScript] callMethod($m): $e'); Lua.pushnil(l); }
		return 1;
	}
	static function _fnDestroy(l:Dynamic):Int
	{
		final h = _handle(l, 1); Lua.settop(l, 0);
		final o = _reg.get(h);
		if (o != null) { try (o:Dynamic).destroy() catch(_) {}; release(h); }
		return 0;
	}
	...
	{
		final h = _handle(l, 1); final front = Lua.gettop(l) > 1 && Lua.toboolean(l, 2);
		Lua.settop(l, 0); final o = _reg.get(h); if (o == null) return 0;
		if (front) flixel.FlxG.state.add(o) else flixel.FlxG.state.insert(0, o); return 0;
	}
	static function _fnRemState(l:Dynamic):Int
	{
		final o = _reg.get(_handle(l, 1)); Lua.settop(l, 0);
		if (o != null) flixel.FlxG.state.remove(o, true); return 0;
	}
	static function _fnAddGroup(l:Dynamic):Int
	{
		final g = _reg.get(_handle(l, 1)); final o = _reg.get(_handle(l, 2)); Lua.settop(l, 0);
		if (g != null && o != null) try (g:Dynamic).add(o) catch(_) {}; return 0;
	}
	static function _fnRemGroup(l:Dynamic):Int
	{
		final g = _reg.get(_handle(l, 1)); final o = _reg.get(_handle(l, 2)); Lua.settop(l, 0);
		if (g != null && o != null) try (g:Dynamic).remove(o, true) catch(_) {}; return 0;
	}
	...
	static function _fnGetPath(l:Dynamic):Int
	{
		...
		_pushOOP(l, _resolvePath(p, _ps())); return 1;
	}
	static function _fnSetPath(l:Dynamic):Int
	{
		final p = Lua.tostring(l, 1); final v = _readOOP(l, 2); Lua.settop(l, 0);
		_applyPath(p, v, _ps()); return 0;
	}
	static function _fnGetOf(l:Dynamic):Int
	{
		final h = _handle(l, 1); final p = Lua.tostring(l, 2); Lua.settop(l, 0);
		_pushOOP(l, _resolvePath(p, _reg.get(h))); return 1;
	}
	static function _fnSetOf(l:Dynamic):Int
	{
		final h = _handle(l, 1); final p = Lua.tostring(l, 2); final v = _readOOP(l, 3);
		Lua.settop(l, 0); _applyPath(p, v, _reg.get(h)); return 0;
	}
	...
	static function _fnCharPos(l:Dynamic):Int
	{
		...
		if (c != null) { c.x = x; c.y = y; } return 0;
	}
	static function _fnCharX(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final x = Lua.tonumber(l, 2); Lua.settop(l, 0);
		if (c != null) c.x = x; return 0;
	}
	static function _fnCharY(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final y = Lua.tonumber(l, 2); Lua.settop(l, 0);
		if (c != null) c.y = y; return 0;
	}
	static function _fnCharGetX(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); Lua.settop(l, 0);
		Lua.pushnumber(l, c != null ? c.x : 0); return 1;
	}
	static function _fnCharGetY(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); Lua.settop(l, 0);
		Lua.pushnumber(l, c != null ? c.y : 0); return 1;
	}
	static function _fnCharScale(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final s = Lua.tonumber(l, 2); Lua.settop(l, 0);
		if (c != null) { c.scale.x = s; c.scale.y = s; } return 0;
	}
	static function _fnCharVisible(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final v = Lua.toboolean(l, 2); Lua.settop(l, 0);
		if (c != null) c.visible = v; return 0;
	}
	static function _fnCharAlpha(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final a = Lua.tonumber(l, 2); Lua.settop(l, 0);
		if (c != null) c.alpha = a; return 0;
	}
	static function _fnCharColor(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final col = Std.int(Lua.tonumber(l, 2)); Lua.settop(l, 0);
		if (c != null) c.color = col; return 0;
	}
	static function _fnCharAngle(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final a = Lua.tonumber(l, 2); Lua.settop(l, 0);
		if (c != null) c.angle = a; return 0;
	}
	static function _fnCharFlip(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final fx = Lua.toboolean(l, 2); Lua.settop(l, 0);
		if (c != null) c.flipX = fx; return 0;
	}
	static function _fnCharScroll(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1));
		final sx = Lua.tonumber(l, 2); final sy = Lua.tonumber(l, 3); Lua.settop(l, 0);
		if (c != null) { c.scrollFactor.x = sx; c.scrollFactor.y = sy; } return 0;
	}
	static function _fnCharGetAnim(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); Lua.settop(l, 0);
		Lua.pushstring(l, c != null ? try c.animation.curAnim.name catch(_) '' : ''); return 1;
	}
	static function _fnCharAnimDone(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); Lua.settop(l, 0);
		Lua.pushboolean(l, c != null ? try c.animation.finished catch(_) false : false); return 1;
	}
	static function _fnCharLock(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final v = Lua.toboolean(l, 2); Lua.settop(l, 0);
		if (c != null) try Reflect.setField(c, 'specialAnim', v) catch(_) {}; return 0;
	}
	static function _fnCharRate(l:Dynamic):Int
	{
		final c = _char(Lua.tostring(l, 1)); final r = Lua.tonumber(l, 2); Lua.settop(l, 0);
		if (c != null) try c.animation.timeScale = r catch(_) {}; return 0;
	}
	static function _fnSetBF(l:Dynamic):Int
	{
		final name = Lua.tostring(l, 1); Lua.settop(l, 0);
		final p = _ps(); if (p != null) try p.changeBF(name) catch(_) {}; return 0;
	}
	static function _fnSetDAD(l:Dynamic):Int
	{
		final name = Lua.tostring(l, 1); Lua.settop(l, 0);
		final p = _ps(); if (p != null) try p.changeDAD(name) catch(_) {}; return 0;
	}
	static function _fnSetGF(l:Dynamic):Int
	{
		final name = Lua.tostring(l, 1); Lua.settop(l, 0);
		final p = _ps(); if (p != null) try p.changeGF(name) catch(_) {}; return 0;
	}
	...
	static function _fnSetHIcon(l:Dynamic):Int
	{
		...
		try { final ic = who == 'player' ? ps.healthIconP1 : ps.healthIconP2;
		      if (ic != null) ic.loadHealthIcon(key); } catch(_) {}; return 0;
	}
	static function _fnSetHIconScale(l:Dynamic):Int
	{
		...
		final ps = _ps(); if (ps == null) return 0;
		try { final ic = who == 'player' ? ps.healthIconP1 : ps.healthIconP2;
		      if (ic != null) ic.setGraphicSize(Std.int(ic.width * sc)); } catch(_) {}; return 0;
	}
	static function _fnSetHIconOffset(l:Dynamic):Int
	{
		...
		try { final ic = who == 'player' ? ps.healthIconP1 : ps.healthIconP2;
		      if (ic != null) { ic.offset.x = dx; ic.offset.y = dy; } } catch(_) {}; return 0;
	}
	...
	static function _fnMakeSprite(l:Dynamic):Int
	{
		...
		if (tag != null) _tags.set(tag, h);
		_pushOOP(l, spr); return 1;
	}
	...
	{
		final s = _spr(l); final p = Lua.tostring(l, 2); Lua.settop(l, 0);
		if (s != null) try s.loadGraphic(Paths.image(p)) catch(_) {}; return 0;
	}
	static function _fnLoadSparrow(l:Dynamic):Int
	{
		final s = _spr(l); final p = Lua.tostring(l, 2); Lua.settop(l, 0);
		if (s != null) try s.frames = Paths.getSparrowAtlas(p) catch(_) {}; return 0;
	}
	static function _fnLoadAtlas(l:Dynamic):Int
	{
		final s = _spr(l); final p = Lua.tostring(l, 2); Lua.settop(l, 0);
		if (s != null) try s.frames = Paths.getPackerAtlas(p) catch(_) {}; return 0;
	}
	static function _fnAddAnim(l:Dynamic):Int
	{
		final s = _spr(l); final n = Lua.tostring(l, 2); final prefix = Lua.tostring(l, 3);
		final fps = Lua.gettop(l) > 3 ? Std.int(Lua.tonumber(l, 4)) : 24;
		final loop = Lua.gettop(l) > 4 && Lua.toboolean(l, 5); Lua.settop(l, 0);
		if (s != null) try s.animation.addByPrefix(n, prefix, fps, loop) catch(_) {}; return 0;
	}
	static function _fnAddAnimOff(l:Dynamic):Int
	{
		final s = _spr(l); final n = Lua.tostring(l, 2);
		final x = Lua.tonumber(l, 3); final y = Lua.tonumber(l, 4); Lua.settop(l, 0);
		if (s != null) try (s:animationdata.FunkinSprite).applyAnimOffset(x, y) catch(_) {}; return 0;
	}
	static function _fnPlayAnim(l:Dynamic):Int
	{
		...
		Lua.settop(l, 0); if (s != null) try s.animation.play(n, f) catch(_) {}; return 0;
	}
	static function _fnStopAnim(l:Dynamic):Int
	{
		final s = _spr(l); Lua.settop(l, 0);
		if (s != null) try s.animation.stop() catch(_) {}; return 0;
	}
	static function _fnAddSpr(l:Dynamic):Int
	{
		final s = _spr(l); final front = Lua.gettop(l) < 2 || Lua.toboolean(l, 2); Lua.settop(l, 0);
		if (s != null) { if (front) flixel.FlxG.state.add(s) else flixel.FlxG.state.insert(0, s); }
		return 0;
	}
	static function _fnRemSpr(l:Dynamic):Int
	{
		final s = _spr(l); Lua.settop(l, 0);
		if (s != null) flixel.FlxG.state.remove(s, true); return 0;
	}
	static function _fnSprScale(l:Dynamic):Int
	{
		final s = _spr(l);
		final sx = Lua.tonumber(l, 2); final sy = Lua.gettop(l) > 2 ? Lua.tonumber(l, 3) : sx;
		Lua.settop(l, 0); if (s != null) { s.scale.x = sx; s.scale.y = sy; } return 0;
	}
	static function _fnSprFlip(l:Dynamic):Int
	{
		...
		Lua.settop(l, 0); if (s != null) { s.flipX = fx; s.flipY = fy; } return 0;
	}
	...
	{
		final s = _spr(l); final x = Lua.tonumber(l, 2); final y = Lua.tonumber(l, 3);
		Lua.settop(l, 0); if (s != null) { s.x = x; s.y = y; } return 0;
	}
	static function _fnSprScroll(l:Dynamic):Int
	{
		final s = _spr(l); final sx = Lua.tonumber(l, 2); final sy = Lua.tonumber(l, 3);
		Lua.settop(l, 0); if (s != null) { s.scrollFactor.x = sx; s.scrollFactor.y = sy; } return 0;
	}
	...
	{
		final s = _spr(l); Lua.settop(l, 0);
		if (s != null) s.screenCenter(); return 0;
	}
	static function _fnSprAngle(l:Dynamic):Int
	{
		final s = _spr(l); final a = Lua.tonumber(l, 2); Lua.settop(l, 0);
		if (s != null) s.angle = a; return 0;
	}
	static function _fnSprVisible(l:Dynamic):Int
	{
		final s = _spr(l); final v = Lua.toboolean(l, 2); Lua.settop(l, 0);
		if (s != null) s.visible = v; return 0;
	}
	static function _fnSprGetX(l:Dynamic):Int { final s = _spr(l); Lua.settop(l,0); Lua.pushnumber(l,s!=null?s.x:0); return 1; }
	static function _fnSprGetY(l:Dynamic):Int { final s = _spr(l); Lua.settop(l,0); Lua.pushnumber(l,s!=null?s.y:0); return 1; }
	static function _fnSprGetW(l:Dynamic):Int { final s = _spr(l); Lua.settop(l,0); Lua.pushnumber(l,s!=null?s.width:0); return 1; }
	static function _fnSprGetH(l:Dynamic):Int { final s = _spr(l); Lua.settop(l,0); Lua.pushnumber(l,s!=null?s.height:0); return 1; }
	static function _fnUpdateHitbox(l:Dynamic):Int
	{
		final s = _spr(l); Lua.settop(l, 0); if (s != null) try s.updateHitbox() catch(_) {}; return 0;
	}
	static function _fnAddAnimIdx(l:Dynamic):Int
	{
		...
		Lua.settop(l, 0);
		if (s != null) try s.animation.addByIndices(n, prefix, indices, '', 24, false) catch(_) {}; return 0;
	}
	static function _fnGetCurAnim(l:Dynamic):Int
	{
		final s = _spr(l); Lua.settop(l, 0);
		Lua.pushstring(l, s != null ? try s.animation.curAnim.name catch(_) '' : ''); return 1;
	}
	static function _fnIsAnimPlay(l:Dynamic):Int
	{
		final s = _spr(l); final n = Lua.tostring(l, 2); Lua.settop(l, 0);
		Lua.pushboolean(l, s != null ? try s.animation.curAnim.name == n catch(_) false : false); return 1;
	}
	static function _fnSetAnimFPS(l:Dynamic):Int
	{
		final s = _spr(l); final fps = Std.int(Lua.tonumber(l, 2)); Lua.settop(l, 0);
		if (s != null) try s.animation.curAnim.frameRate = fps catch(_) {}; return 0;
	}
	static function _fnSprCam(l:Dynamic):Int
	{
		final s = _spr(l); final camName = Lua.tostring(l, 2); Lua.settop(l, 0);
		if (s != null) try {
	...
	{
		final s = _spr(l); final w = Std.int(Lua.tonumber(l, 2)); final h2 = Std.int(Lua.tonumber(l, 3));
		Lua.settop(l, 0); if (s != null) try s.setGraphicSize(w, h2) catch(_) {}; return 0;
	}

	// Text
	static function _fnMakeText(l:Dynamic):Int
	{
		final x = Lua.tonumber(l, 1); final y = Lua.tonumber(l, 2); final w = Std.int(Lua.tonumber(l, 3));
		final txt = Lua.tostring(l, 4); final size = Lua.gettop(l) > 4 ? Std.int(Lua.tonumber(l, 5)) : 16;
		Lua.settop(l, 0);
		final t = new flixel.text.FlxText(x, y, w, txt, size);
		_pushOOP(l, t); return 1;
	}
	static function _fnSetText(l:Dynamic):Int
	{
		final s = _spr(l); final t = Lua.tostring(l, 2); Lua.settop(l, 0);
		if (s != null) try (s:flixel.text.FlxText).text = t catch(_) {}; return 0;
	}
	static function _fnTextSize(l:Dynamic):Int
	{
		final s = _spr(l); final sz = Std.int(Lua.tonumber(l, 2)); Lua.settop(l, 0);
		if (s != null) try (s:flixel.text.FlxText).size = sz catch(_) {}; return 0;
	}
	static function _fnTextFont(l:Dynamic):Int
	{
		final s = _spr(l); final f = Lua.tostring(l, 2); Lua.settop(l, 0);
		if (s != null) try (s:flixel.text.FlxText).font = f catch(_) {}; return 0;
	}
	static function _fnTextBold(l:Dynamic):Int
	{
		final s = _spr(l); final b = Lua.toboolean(l, 2); Lua.settop(l, 0);
		if (s != null) try (s:flixel.text.FlxText).bold = b catch(_) {}; return 0;
	}
	static function _fnTextAlign(l:Dynamic):Int
	{
		final s = _spr(l); final a = Lua.tostring(l, 2); Lua.settop(l, 0);
		if (s != null) try (s:flixel.text.FlxText).alignment = cast a catch(_) {}; return 0;
	}
	static function _fnTextBorder(l:Dynamic):Int
	{
		final s = _spr(l); final sz = Lua.tonumber(l, 2); final c = Std.int(Lua.tonumber(l, 3));
		Lua.settop(l, 0);
		if (s != null) try {
	...
	{
		final s = _spr(l); final c = Std.int(Lua.tonumber(l, 2)); Lua.settop(l, 0);
		if (s != null) try (s:flixel.text.FlxText).color = c catch(_) {}; return 0;
	}
	static function _fnTextItalic(l:Dynamic):Int
	{
		final s = _spr(l); final v = Lua.toboolean(l, 2); Lua.settop(l, 0);
		if (s != null) try (s:flixel.text.FlxText).italic = v catch(_) {}; return 0;
	}
	static function _fnTextShadow(l:Dynamic):Int
	{
		final s = _spr(l); final c = Std.int(Lua.tonumber(l, 2)); final sz = Lua.tonumber(l, 3);
		Lua.settop(l, 0);
		if (s != null) try {
			final t:flixel.text.FlxText = cast s;
	...
	{
		final s = _spr(l); Lua.settop(l, 0);
		Lua.pushstring(l, s != null ? try (s:flixel.text.FlxText).text catch(_) '' : ''); return 1;
	}

	// Camera
	static function _fnCamZoom(l:Dynamic):Int
	{
		final z = Lua.tonumber(l, 1); Lua.settop(l, 0); flixel.FlxG.camera.zoom = z; return 0;
	}
	static function _fnCamZoomTween(l:Dynamic):Int
	{
		final z = Lua.tonumber(l, 1); final dur = Lua.tonumber(l, 2);
		final ease = Lua.gettop(l) > 2 ? Lua.tostring(l, 3) : 'linear'; Lua.settop(l, 0);
		flixel.tweens.FlxTween.tween(flixel.FlxG.camera, {zoom: z}, dur, {ease: _ease(ease)});
		return 0;
	}
	static function _fnCamFlash(l:Dynamic):Int
	{
		final c = Lua.tostring(l, 1); final dur = Lua.gettop(l) > 1 ? Lua.tonumber(l, 2) : 0.5;
		Lua.settop(l, 0); flixel.FlxG.camera.flash(flixel.util.FlxColor.fromString(c ?? 'white'), dur); return 0;
	}
	static function _fnCamShake(l:Dynamic):Int
	{
		final i = Lua.tonumber(l, 1); final dur = Lua.tonumber(l, 2); Lua.settop(l, 0);
		flixel.FlxG.camera.shake(i, dur); return 0;
	}
	static function _fnCamFade(l:Dynamic):Int
	{
		final c = Lua.tostring(l, 1); final dur = Lua.gettop(l) > 1 ? Lua.tonumber(l, 2) : 1.0;
		Lua.settop(l, 0); flixel.FlxG.camera.fade(flixel.util.FlxColor.fromString(c ?? 'black'), dur); return 0;
	}
	static function _fnCamPan(l:Dynamic):Int
	{
		final x = Lua.tonumber(l, 1); final y = Lua.tonumber(l, 2); final dur = Lua.tonumber(l, 3);
		Lua.settop(l, 0);
		flixel.tweens.FlxTween.tween(flixel.FlxG.camera.scroll, {x: x, y: y}, dur); return 0;
	}
	static function _fnCamSnap(l:Dynamic):Int
	{
		final x = Lua.tonumber(l, 1); final y = Lua.tonumber(l, 2); Lua.settop(l, 0);
		flixel.FlxG.camera.scroll.set(x, y); return 0;
	}
	static function _fnCamHandle(l:Dynamic):Int
	{
		final name = Lua.gettop(l) > 0 ? Lua.tostring(l, 1) : 'game'; Lua.settop(l, 0);
	...
	{
		final o = _reg.get(_handle(l, 1)); Lua.settop(l, 0);
		if (o != null) flixel.FlxG.camera.follow(o); return 0;
	}
	static function _fnCamFollow(l:Dynamic):Int
	{
		final style = Lua.tostring(l, 1); Lua.settop(l, 0);
		...
		flixel.FlxG.camera.follow(flixel.FlxG.camera.target, s); return 0;
	}
	static function _fnCamLerp(l:Dynamic):Int
	{
		final v = Lua.tonumber(l, 1); Lua.settop(l, 0); flixel.FlxG.camera.followLerp = v; return 0;
	}
	static function _fnGetCamZoom(l:Dynamic):Int
	{
		Lua.settop(l, 0); Lua.pushnumber(l, flixel.FlxG.camera.zoom); return 1;
	}
	static function _fnCamScrollX(l:Dynamic):Int
	{
		final v = Lua.tonumber(l, 1); Lua.settop(l, 0); flixel.FlxG.camera.scroll.x = v; return 0;
	}
	static function _fnCamScrollY(l:Dynamic):Int
	{
		final v = Lua.tonumber(l, 1); Lua.settop(l, 0); flixel.FlxG.camera.scroll.y = v; return 0;
	}
	static function _fnRemoveCam(l:Dynamic):Int
	{
		final c = _reg.get(_handle(l, 1)); Lua.settop(l, 0);
		if (c != null) try flixel.FlxG.cameras.remove(c) catch(_) {}; return 0;
	}

	// Tweens
	static function _fnTweenProp(l:Dynamic):Int
	{
		final h = _handle(l, 1); final field = Lua.tostring(l, 2);
		final to = Lua.tonumber(l, 3); final dur = Lua.tonumber(l, 4);
		final ease = Lua.gettop(l) > 4 ? Lua.tostring(l, 5) : 'linear'; Lua.settop(l, 0);
		final obj = _reg.get(h); if (obj == null) return 0;
		final props:Dynamic = {}; Reflect.setField(props, field, to);
		final tw = flixel.tweens.FlxTween.tween(obj, props, dur, {ease: _ease(ease)});
		_pushOOP(l, tw); return 1;
	}
	static function _fnTweenColor(l:Dynamic):Int
	{
		final h = _handle(l, 1); final dur = Lua.tonumber(l, 2);
		final from = Std.int(Lua.tonumber(l, 3)); final to = Std.int(Lua.tonumber(l, 4));
		Lua.settop(l, 0); final obj = _reg.get(h); if (obj == null) return 0;
	...
	{
		final h = _handle(l, 1); Lua.settop(l, 0);
		final tw = _reg.get(h); if (tw != null) try tw.cancel() catch(_) {}; return 0;
	}
	static function _fnTweenAngle(l:Dynamic):Int
	{
		final h = _handle(l, 1); final to = Lua.tonumber(l, 2); final dur = Lua.tonumber(l, 3);
		final ease = Lua.gettop(l) > 3 ? Lua.tostring(l, 4) : 'linear'; Lua.settop(l, 0);
		final obj = _reg.get(h); if (obj == null) return 0;
	...
	{
		final h = _handle(l, 1); final tx = Lua.tonumber(l, 2); final ty = Lua.tonumber(l, 3);
		final dur = Lua.tonumber(l, 4); final ease = Lua.gettop(l) > 4 ? Lua.tostring(l, 5) : 'linear';
		Lua.settop(l, 0); final obj = _reg.get(h); if (obj == null) return 0;
	...
	static function _fnNumTween(l:Dynamic):Int
	{
		...
		final cbName = Lua.tostring(l, 4); final ease = Lua.gettop(l) > 4 ? Lua.tostring(l, 5) : 'linear';
		Lua.settop(l, 0);
		...
		final tw = flixel.tweens.FlxTween.num(from, to, dur, {ease: _ease(ease)}, function(v:Float) {
			...
			if (Lua.type(luaRef, -1) == 6) { Lua.pushnumber(luaRef, v); Lua.pcall(luaRef, 1, 0, 0); }
			else Lua.pop(luaRef, 1);
		});
		_pushOOP(l, tw); return 1;
	}
	...
	{
		final dur = Lua.tonumber(l, 1); final cbName = Lua.tostring(l, 2);
		...
		t.start(dur, function(_) {
			Lua.getglobal(luaRef, cbName);
			if (Lua.type(luaRef, -1) == 6) Lua.pcall(luaRef, 0, 0, 0) else Lua.pop(luaRef, 1);
		}, loops);
		_pushOOP(l, t); return 1;
	}
	...
	{
		final h = _handle(l, 1); Lua.settop(l, 0);
		final t = _reg.get(h); if (t != null) try t.cancel() catch(_) {}; return 0;
	}
	...
	static function _fnCutNew(l:Dynamic):Int
	{
		...
		Lua.settop(l, 0);
		try { final b = new funkin.cutscenes.SpriteCutscene(flixel.FlxG.state, key ?? '', song); _pushOOP(l, b); }
		catch(_) { Lua.pushnil(l); }
		return 1;
	}
	static function _fnCutSkip(l:Dynamic):Int
	{
		final h = _handle(l, 1); final v = Lua.toboolean(l, 2); Lua.settop(l, 0);
		final b = _reg.get(h); if (b != null) try b.skippable = v catch(_) {}; return 0;
	}
	static function _fnCutRect(l:Dynamic):Int
	{
		final h = _handle(l, 1); final name = Lua.tostring(l, 2); final col = Lua.tostring(l, 3);
		Lua.settop(l, 0); final b = _reg.get(h);
		if (b != null) try b.defineRect(name, flixel.util.FlxColor.fromString(col ?? 'black')) catch(_) {};
		return 0;
	}
	static function _fnCutSpr(l:Dynamic):Int
	{
		final h = _handle(l, 1); final name = Lua.tostring(l, 2); final path = Lua.tostring(l, 3);
		Lua.settop(l, 0); final b = _reg.get(h);
		if (b != null) try b.defineSprite(name, path) catch(_) {}; return 0;
	}
	static function _fnCutAdd(l:Dynamic):Int
	{
		final h = _handle(l, 1); final name = Lua.tostring(l, 2); Lua.settop(l, 0);
		final b = _reg.get(h); if (b != null) try b.add(name) catch(_) {}; return 0;
	}
	static function _fnCutRem(l:Dynamic):Int
	{
		final h = _handle(l, 1); final name = Lua.tostring(l, 2); Lua.settop(l, 0);
		final b = _reg.get(h); if (b != null) try b.remove(name) catch(_) {}; return 0;
	}
	static function _fnCutWait(l:Dynamic):Int
	{
		final h = _handle(l, 1); final dur = Lua.tonumber(l, 2); Lua.settop(l, 0);
		final b = _reg.get(h); if (b != null) try b.wait(dur) catch(_) {}; return 0;
	}
	static function _fnCutAnim(l:Dynamic):Int
	{
		final h = _handle(l, 1); final who = Lua.tostring(l, 2); final anim = Lua.tostring(l, 3);
		Lua.settop(l, 0); final b = _reg.get(h);
		if (b != null) try b.playCharAnim(who, anim) catch(_) {}; return 0;
	}
	static function _fnCutSound(l:Dynamic):Int
	{
		final h = _handle(l, 1); final path = Lua.tostring(l, 2); Lua.settop(l, 0);
		final b = _reg.get(h); if (b != null) try b.playSound(path) catch(_) {}; return 0;
	}
	static function _fnCutCamZ(l:Dynamic):Int
	{
		final h = _handle(l, 1); final z = Lua.tonumber(l, 2); final dur = Lua.tonumber(l, 3);
		Lua.settop(l, 0); final b = _reg.get(h);
		if (b != null) try b.cameraZoom(z, dur) catch(_) {}; return 0;
	}
	static function _fnCutCamF(l:Dynamic):Int
	{
		final h = _handle(l, 1); final col = Lua.tostring(l, 2); final dur = Lua.tonumber(l, 3);
		Lua.settop(l, 0); final b = _reg.get(h);
		if (b != null) try b.cameraFlash(flixel.util.FlxColor.fromString(col ?? 'white'), dur) catch(_) {};
		return 0;
	}
	static function _fnCutPlay(l:Dynamic):Int
	{
		final h = _handle(l, 1); Lua.settop(l, 0);
		final b = _reg.get(h); if (b != null) try b.play() catch(_) {}; return 0;
	}
	...
	static function _fnEndSong(l:Dynamic):Int   { Lua.settop(l,0); final p=_ps(); if(p!=null)p.endSong(); return 0; }
	static function _fnGameOver(l:Dynamic):Int  { Lua.settop(l,0); final p=_ps(); if(p!=null)p.health=0; return 0; }
	static function _fnPause(l:Dynamic):Int     { Lua.settop(l,0); final p=_ps(); if(p!=null)try p.pauseSong() catch(_) flixel.FlxG.timeScale=0; return 0; }
	static function _fnResume(l:Dynamic):Int    { Lua.settop(l,0); final p=_ps(); if(p!=null)try p.resumeSong() catch(_) flixel.FlxG.timeScale=1; return 0; }

	// Notes
	static function _fnSpawnNote(l:Dynamic):Int
	{
		final t=Lua.tonumber(l,1); final d=Std.int(Lua.tonumber(l,2));
		final len=Lua.gettop(l)>2?Lua.tonumber(l,3):0.0; Lua.settop(l,0);
		final p=_ps(); if(p!=null) try p.spawnNote(t,d,len) catch(_) {}; return 0;
	}
	static function _fnNoteDir(l:Dynamic):Int   { final h=_handle(l,1); Lua.settop(l,0); final n=_reg.get(h); Lua.pushnumber(l,n!=null?try n.noteData catch(_) 0:0); return 1; }
	static function _fnNoteTime(l:Dynamic):Int  { final h=_handle(l,1); Lua.settop(l,0); final n=_reg.get(h); Lua.pushnumber(l,n!=null?try n.strumTime catch(_) 0:0); return 1; }
	static function _fnNoteAlpha(l:Dynamic):Int { final a=Lua.tonumber(l,1); Lua.settop(l,0); final p=_ps(); if(p!=null)try p.noteAlphaOverride=a catch(_) {}; return 0; }
	static function _fnNoteColor(l:Dynamic):Int { final c=Std.int(Lua.tonumber(l,1)); Lua.settop(l,0); final p=_ps(); if(p!=null)try p.noteColorOverride=c catch(_) {}; return 0; }
	static function _fnSkipNote(l:Dynamic):Int  { final h=_handle(l,1); Lua.settop(l,0); final n=_reg.get(h); if(n!=null)try n.ignoreNote=true catch(_) {}; return 0; }
	static function _fnNoteSkin(l:Dynamic):Int
	{
		final skin=Lua.tostring(l,1); Lua.settop(l,0);
		final p=_ps(); if(p!=null)try p.noteSkinOverride=skin catch(_) {}; return 0;
	}
	static function _fnNoteSplash(l:Dynamic):Int
	{
		final d=Std.int(Lua.tonumber(l,1)); Lua.settop(l,0);
		final p=_ps(); if(p!=null)try p.spawnSplash(d) catch(_) {}; return 0;
	}
	static function _fnHoldActive(l:Dynamic):Int { final h=_handle(l,1); Lua.settop(l,0); final n=_reg.get(h); Lua.pushboolean(l,n!=null?try n.held catch(_) false:false); return 1; }
	static function _fnForNote(l:Dynamic):Int
	{
		final cbName = Lua.tostring(l, 1); Lua.settop(l, 0);
		final p=_ps(); if(p==null) return 0;
		final luaRef=_sCurrentLua;
		try { final noteGroup = (p.notes : flixel.group.FlxGroup.FlxTypedGroup<funkin.gameplay.notes.Note>);
		for(n in noteGroup.members) {
			Lua.getglobal(luaRef, cbName);
			if(Lua.type(luaRef,-1)==6) { _pushOOP(luaRef,n); Lua.pcall(luaRef,1,0,0); } else Lua.pop(luaRef,1);
		} } catch(_) {}; return 0;
	}
	static function _fnNoteSkinHandle(l:Dynamic):Int
	{
		Lua.settop(l, 0);
		_pushOOP(l, funkin.gameplay.notes.NoteSkinSystem); return 1;
	}
	...
	static function _fnApplyRGB(l:Dynamic):Int
	{
		...
		if (spr != null) try funkin.gameplay.notes.NoteSkinSystem.applyRGBShader(spr, dir, mul) catch(_) {};
		return 0;
	}
	...
	static function _fnApplyRGBColor(l:Dynamic):Int
	{
		...
		Lua.settop(l, 0);
		final spr = _reg.get(h);
		if (spr != null) try funkin.gameplay.notes.NoteSkinSystem.applyRGBColor(spr, col, mul) catch(_) {};
		return 0;
	}
	...
	static function _fnSetRGBIntensity(l:Dynamic):Int
	{
		...
		Lua.settop(l, 0);
		final spr = _reg.get(h);
		if (spr != null) try funkin.gameplay.notes.NoteSkinSystem.setRGBIntensity(spr, mul) catch(_) {};
		return 0;
	}
	...
	static function _fnRemoveRGB(l:Dynamic):Int
	{
		final h = _handle(l, 1); Lua.settop(l, 0);
		final spr = _reg.get(h);
		if (spr != null) try funkin.gameplay.notes.NoteSkinSystem.removeRGBShader(spr) catch(_) {};
		return 0;
	}
	...
	static function _fnTweenRGBDir(l:Dynamic):Int
	{
		...
		if (spr != null) try {
			...
			if (tw != null) _pushOOP(l, tw) else Lua.pushnil(l);
		} catch(_) { Lua.pushnil(l); }
		return 1;
	}
	...
	static function _fnRegNoteType(l:Dynamic):Int
	{
		...
		final onHit:Dynamic = cbHit != null ? function(note:Dynamic) {
			Lua.getglobal(luaRef, cbHit); if(Lua.type(luaRef,-1)==6) { _pushOOP(luaRef,note); Lua.pcall(luaRef,1,0,0); } else Lua.pop(luaRef,1);
		} : null;
		try funkin.gameplay.notes.NoteTypeManager.register(name, { onHit: onHit }) catch(_) {};
		return 0;
	}
	...
	{
		final p=Lua.tostring(l,1); final v=Lua.gettop(l)>1?Lua.tonumber(l,2):1.0;
		final loop=Lua.gettop(l)<3||Lua.toboolean(l,3); Lua.settop(l,0);
		try flixel.FlxG.sound.playMusic(Paths.music(p), v, loop) catch(_) {}; return 0;
	}
	static function _fnStopMusic(l:Dynamic):Int  { Lua.settop(l,0); try flixel.FlxG.sound.music.stop() catch(_) {}; return 0; }
	static function _fnPauseMusic(l:Dynamic):Int { Lua.settop(l,0); try flixel.FlxG.sound.music.pause() catch(_) {}; return 0; }
	static function _fnResumeMusic(l:Dynamic):Int{ Lua.settop(l,0); try flixel.FlxG.sound.music.resume() catch(_) {}; return 0; }
	static function _fnPlaySound(l:Dynamic):Int
	{
		final p=Lua.tostring(l,1); final v=Lua.gettop(l)>1?Lua.tonumber(l,2):1.0; Lua.settop(l,0);
		try flixel.FlxG.sound.play(Paths.sound(p), v) catch(_) {}; return 0;
	}
	static function _fnMusicPos(l:Dynamic):Int  { Lua.settop(l,0); Lua.pushnumber(l, try flixel.FlxG.sound.music.time catch(_) 0); return 1; }
	static function _fnSetMusicPos(l:Dynamic):Int{ final t=Lua.tonumber(l,1); Lua.settop(l,0); try flixel.FlxG.sound.music.time=t catch(_) {}; return 0; }
	static function _fnMusicPitch(l:Dynamic):Int { final p=Lua.tonumber(l,1); Lua.settop(l,0); try flixel.FlxG.sound.music.pitch=p catch(_) {}; return 0; }
	static function _fnVocVol(l:Dynamic):Int
	{
		final v=Lua.tonumber(l,1); Lua.settop(l,0);
		final p=_ps(); if(p!=null)try { p.vocals.volume=v; } catch(_) {}; return 0;
	}
	static function _fnVocP(l:Dynamic):Int   { final s=Lua.tostring(l,1); Lua.settop(l,0); final p=_ps(); if(p!=null)try p.loadVocals(s,'player') catch(_) {}; return 0; }
	static function _fnVocOp(l:Dynamic):Int  { final s=Lua.tostring(l,1); Lua.settop(l,0); final p=_ps(); if(p!=null)try p.loadVocals(s,'opponent') catch(_) {}; return 0; }
	static function _fnMuteVoc(l:Dynamic):Int{ final v=Lua.toboolean(l,1); Lua.settop(l,0); final p=_ps(); if(p!=null)try p.vocals.volume=v?0:1 catch(_) {}; return 0; }

	// Config
	static function _fnSetConfig(l:Dynamic):Int
	{
		final k=Lua.tostring(l,1); final v=_readOOP(l,2); Lua.settop(l,0);
		try Reflect.setField(funkin.data.GlobalConfig, k, v) catch(_) {}; return 0;
	}
	static function _fnGetConfig(l:Dynamic):Int
	{
		final k=Lua.tostring(l,1); Lua.settop(l,0);
		_pushOOP(l, try Reflect.field(funkin.data.GlobalConfig, k) catch(_) null); return 1;
	}

	// Input
	static function _fnKeyP(l:Dynamic):Int  { final k=_key(Lua.tostring(l,1)); Lua.settop(l,0); Lua.pushboolean(l,k!=null&&flixel.FlxG.keys.checkStatus(k, flixel.input.FlxInput.FlxInputState.PRESSED)); return 1; }
	static function _fnKeyJP(l:Dynamic):Int { final k=_key(Lua.tostring(l,1)); Lua.settop(l,0); Lua.pushboolean(l,k!=null&&flixel.FlxG.keys.checkStatus(k, flixel.input.FlxInput.FlxInputState.JUST_PRESSED)); return 1; }
	static function _fnKeyJR(l:Dynamic):Int { final k=_key(Lua.tostring(l,1)); Lua.settop(l,0); Lua.pushboolean(l,k!=null&&flixel.FlxG.keys.checkStatus(k, flixel.input.FlxInput.FlxInputState.JUST_RELEASED)); return 1; }
	static function _fnMouseX(l:Dynamic):Int { Lua.settop(l,0); Lua.pushnumber(l,flixel.FlxG.mouse.x); return 1; }
	static function _fnMouseY(l:Dynamic):Int { Lua.settop(l,0); Lua.pushnumber(l,flixel.FlxG.mouse.y); return 1; }
	static function _fnMouseP(l:Dynamic):Int  { Lua.settop(l,0); Lua.pushboolean(l,flixel.FlxG.mouse.pressed); return 1; }
	static function _fnMouseJP(l:Dynamic):Int { Lua.settop(l,0); Lua.pushboolean(l,flixel.FlxG.mouse.justPressed); return 1; }
	static function _fnMouseRP(l:Dynamic):Int  { Lua.settop(l,0); Lua.pushboolean(l,flixel.FlxG.mouse.pressedRight); return 1; }
	static function _fnMouseRJP(l:Dynamic):Int { Lua.settop(l,0); Lua.pushboolean(l,flixel.FlxG.mouse.justPressedRight); return 1; }
	static function _fnPadP(l:Dynamic):Int
	{
		final b=Lua.tostring(l,1); Lua.settop(l,0);
		try { final gb=flixel.input.gamepad.FlxGamepadInputID.fromString(b); Lua.pushboolean(l,flixel.FlxG.gamepads.anyPressed(gb)); } catch(_) { Lua.pushboolean(l,false); }
		return 1;
	}
	static function _fnPadJP(l:Dynamic):Int
	{
		final b=Lua.tostring(l,1); Lua.settop(l,0);
		try { final gb=flixel.input.gamepad.FlxGamepadInputID.fromString(b); Lua.pushboolean(l,flixel.FlxG.gamepads.anyJustPressed(gb)); } catch(_) { Lua.pushboolean(l,false); }
		return 1;
	}
	...
	static function _fnTrace(l:Dynamic):Int
	{
		...
		final parts=[for (i in 0...n) Lua.tostring(l,i+1) ?? 'nil']; Lua.settop(l,0);
		trace('[Lua:$_sCurrentId] ' + parts.join('\t')); return 0;
	}
	static function _fnBeat(l:Dynamic):Int    { Lua.settop(l,0); Lua.pushnumber(l, try Math.floor(funkin.data.Conductor.songPosition / funkin.data.Conductor.crochet) catch(_) 0);     return 1; }
	static function _fnStep(l:Dynamic):Int    { Lua.settop(l,0); Lua.pushnumber(l, try Math.floor(funkin.data.Conductor.songPosition / funkin.data.Conductor.stepCrochet) catch(_) 0); return 1; }
	static function _fnBPM(l:Dynamic):Int     { Lua.settop(l,0); Lua.pushnumber(l, try funkin.data.Conductor.bpm catch(_) 100);      return 1; }
	static function _fnSongPos(l:Dynamic):Int { Lua.settop(l,0); Lua.pushnumber(l, try funkin.data.Conductor.songPosition catch(_) 0); return 1; }
	static function _fnRndInt(l:Dynamic):Int  { final a=Std.int(Lua.tonumber(l,1)); final b=Std.int(Lua.tonumber(l,2)); Lua.settop(l,0); Lua.pushnumber(l,a+Std.int(Math.random()*(b-a+1))); return 1; }
	static function _fnRndFlt(l:Dynamic):Int  { final a=Lua.tonumber(l,1); final b=Lua.tonumber(l,2); Lua.settop(l,0); Lua.pushnumber(l,a+Math.random()*(b-a)); return 1; }
	...
	static function _fnHex(l:Dynamic):Int     { final h=Lua.tostring(l,1); Lua.settop(l,0); try Lua.pushnumber(l,flixel.util.FlxColor.fromString('#$h')) catch(_) Lua.pushnumber(l,0); return 1; }
	static function _fnLerp(l:Dynamic):Int    { final a=Lua.tonumber(l,1); final b=Lua.tonumber(l,2); final r=Lua.tonumber(l,3); Lua.settop(l,0); Lua.pushnumber(l,a+(b-a)*r); return 1; }
	static function _fnClamp(l:Dynamic):Int   { final v=Lua.tonumber(l,1); final mn=Lua.tonumber(l,2); final mx=Lua.tonumber(l,3); Lua.settop(l,0); Lua.pushnumber(l,Math.min(mx,Math.max(mn,v))); return 1; }
	static function _fnFileEx(l:Dynamic):Int  { final p=Lua.tostring(l,1); Lua.settop(l,0); Lua.pushboolean(l,#if sys FileSystem.exists(p) #else false #end); return 1; }
	static function _fnFileR(l:Dynamic):Int   { final p=Lua.tostring(l,1); Lua.settop(l,0); try Lua.pushstring(l,#if sys File.getContent(p) #else '' #end) catch(_) Lua.pushnil(l); return 1; }
	static function _fnFileW(l:Dynamic):Int   { final p=Lua.tostring(l,1); final c=Lua.tostring(l,2); Lua.settop(l,0); #if sys try File.saveContent(p,c) catch(_) {} #end return 0; }
	static function _fnSongName(l:Dynamic):Int   { Lua.settop(l,0); Lua.pushstring(l, try funkin.gameplay.PlayState.SONG.song   catch(_) ''); return 1; }
	static function _fnSongArtist(l:Dynamic):Int { Lua.settop(l,0); Lua.pushstring(l, try funkin.gameplay.PlayState.SONG.artist catch(_) ''); return 1; }
	static function _fnIsStory(l:Dynamic):Int    { Lua.settop(l,0); Lua.pushboolean(l, try funkin.gameplay.PlayState.isStoryMode catch(_) false); return 1; }
	static function _fnGetDiff(l:Dynamic):Int    { Lua.settop(l,0); Lua.pushstring(l, try Std.string(funkin.gameplay.PlayState.storyDifficulty) catch(_) 'normal'); return 1; }
	static function _fnGetAcc(l:Dynamic):Int     { final p=_ps(); Lua.settop(l,0); Lua.pushnumber(l,p!=null?try p.scoreManager.accuracy catch(_) 0:0); return 1; }
	static function _fnGetSicks(l:Dynamic):Int   { final p=_ps(); Lua.settop(l,0); Lua.pushnumber(l,p!=null?try p.scoreManager.sicks catch(_) 0:0); return 1; }
	static function _fnGetGoods(l:Dynamic):Int   { final p=_ps(); Lua.settop(l,0); Lua.pushnumber(l,p!=null?try p.scoreManager.goods catch(_) 0:0); return 1; }
	static function _fnGetBads(l:Dynamic):Int    { final p=_ps(); Lua.settop(l,0); Lua.pushnumber(l,p!=null?try p.scoreManager.bads catch(_) 0:0); return 1; }
	static function _fnGetShits(l:Dynamic):Int   { final p=_ps(); Lua.settop(l,0); Lua.pushnumber(l,p!=null?try p.scoreManager.shits catch(_) 0:0); return 1; }
	static function _fnSetSicks(l:Dynamic):Int   { final v=Std.int(Lua.tonumber(l,1)); Lua.settop(l,0); final p=_ps(); if(p!=null)try p.scoreManager.sicks=v catch(_) {}; return 0; }
	static function _fnSetScroll(l:Dynamic):Int  { final v=Lua.tonumber(l,1); Lua.settop(l,0); final p=_ps(); if(p!=null)try p.scrollSpeed=v catch(_) {}; return 0; }
	static function _fnGetScroll(l:Dynamic):Int  { final p=_ps(); Lua.settop(l,0); Lua.pushnumber(l,p!=null?try p.scrollSpeed catch(_) 1:1); return 1; }

	// Shared / script communication
	static var _sharedData:Map<String, Dynamic> = new Map();
	static function _fnSetShared(l:Dynamic):Int  { final k=Lua.tostring(l,1); final v=_readOOP(l,2); Lua.settop(l,0); _sharedData.set(k,v); return 0; }
	static function _fnGetShared(l:Dynamic):Int  { final k=Lua.tostring(l,1); Lua.settop(l,0); _pushOOP(l,_sharedData.get(k)); return 1; }
	static function _fnDelShared(l:Dynamic):Int  { final k=Lua.tostring(l,1); Lua.settop(l,0); _sharedData.remove(k); return 0; }
	static function _fnBroadcast(l:Dynamic):Int
	{
		final ev=Lua.tostring(l,1); final nArg=Lua.gettop(l)-1;
		final args=[for(i in 0...nArg) _readOOP(l,i+2)]; Lua.settop(l,0);
		try funkin.scripting.ScriptHandler.callOnScripts(ev,args) catch(_) {}; return 0;
	}
	static function _fnCallScripts(l:Dynamic):Int  { return _fnBroadcast(l); }
	static function _fnSetScriptVar(l:Dynamic):Int  { final id2=Lua.tostring(l,1); final k=Lua.tostring(l,2); final v=_readOOP(l,3); Lua.settop(l,0); try funkin.scripting.ScriptHandler.setOnScripts(k,v) catch(_) {}; return 0; }
	static function _fnGetScriptVar(l:Dynamic):Int  { final id2=Lua.tostring(l,1); final k=Lua.tostring(l,2); Lua.settop(l,0); _pushOOP(l,try funkin.scripting.ScriptHandler.getFromScripts(k,null) catch(_) null); return 1; }

	// Modchart
	static function _fnSetMod(l:Dynamic):Int
	{
		final name=Lua.tostring(l,1); final v=Lua.tonumber(l,2); Lua.settop(l,0);
	...
	static function _fnGetMod(l:Dynamic):Int
	{
		...
		} catch(_) { Lua.pushnumber(l, 0); } return 1;
	}
	static function _fnClearMods(l:Dynamic):Int { Lua.settop(l,0); try funkin.gameplay.modchart.ModChartManager.instance.clearEvents() catch(_) {}; return 0; }
	static function _fnNoteMod(l:Dynamic):Int
	{
		final h=_handle(l,1); final mod=Lua.tostring(l,2); final v=Lua.tonumber(l,3); Lua.settop(l,0);
		final n=_reg.get(h); if(n!=null)try Reflect.setField(n,'modifier_'+mod,v) catch(_) {}; return 0;
	}

	// Events
	static function _fnTriggerEv(l:Dynamic):Int
	{
		final name=Lua.tostring(l,1); final v1=Lua.gettop(l)>1?Lua.tostring(l,2):''; final v2=Lua.gettop(l)>2?Lua.tostring(l,3):'';
		Lua.settop(l,0); try funkin.scripting.events.EventManager.fireEvent(name, v1 ?? '', v2 ?? '') catch(_) {}; return 0;
	}
	static function _fnRegisterEv(l:Dynamic):Int
	{
		final name=Lua.tostring(l,1); final cb=Lua.tostring(l,2); Lua.settop(l,0);
		final luaRef=_sCurrentLua; try funkin.scripting.events.EventManager.registerCustomEvent(name, function(evts) {
			Lua.getglobal(luaRef,cb);
			final v1 = evts.length > 0 ? (evts[0].value1 ?? '') : '';
			final v2 = evts.length > 0 ? (evts[0].value2 ?? '') : '';
			if(Lua.type(luaRef,-1)==6) {Lua.pushstring(luaRef,v1); Lua.pushstring(luaRef,v2); Lua.pcall(luaRef,2,0,0);} else Lua.pop(luaRef,1);
			return false;
		}) catch(_) {}; return 0;
	}
	static function _fnGetEventDef(l:Dynamic):Int   { final name=Lua.tostring(l,1); Lua.settop(l,0); _pushOOP(l,try funkin.scripting.events.EventRegistry.get(name) catch(_) null); return 1; }
	static function _fnListEvents(l:Dynamic):Int     { Lua.settop(l,0); _pushOOP(l,try funkin.scripting.events.EventRegistry.eventList catch(_) null); return 1; }
	static function _fnRegisterEventDef(l:Dynamic):Int { Lua.settop(l,0); return 0; }

	// Shaders
	static function _fnAddShader(l:Dynamic):Int
	{
		final h=_handle(l,1); final key=Lua.tostring(l,2); Lua.settop(l,0);
		final obj=_reg.get(h); if(obj==null)return 0;
		try funkin.graphics.shaders.ShaderManager.applyToNote(obj, 0) catch(_) {}; return 0;
	}
	static function _fnRemoveShader(l:Dynamic):Int
	{
		final h=_handle(l,1); final key=Lua.tostring(l,2); Lua.settop(l,0);
		final obj=_reg.get(h); if(obj==null)return 0;
		try funkin.graphics.shaders.ShaderManager.removeFromNote(obj) catch(_) {}; return 0;
	}
	static function _fnShaderProp(l:Dynamic):Int
	{
		final h=_handle(l,1); final key=Lua.tostring(l,2); final prop=Lua.tostring(l,3); final v=_readOOP(l,4); Lua.settop(l,0);
		try {
			...
			if (shader != null) Reflect.setField(shader, prop, v);
		} catch(_) {}; return 0;
	}
	...
	{
		final msg=Lua.tostring(l,1); final dur=Lua.gettop(l)>1?Lua.tonumber(l,2):2.0; Lua.settop(l,0);
		try { final d = new funkin.scripting.ScriptDialog(); d.addLine('', msg, dur); d.close(); } catch(_) {}; return 0;
	}
	static function _fnNewDialog(l:Dynamic):Int
	{
		Lua.settop(l,0); final d=try new funkin.scripting.ScriptDialog() catch(_) null;
		if(d!=null) _pushOOP(l,d) else Lua.pushnil(l); return 1;
	}
	static function _fnDialogAddLine(l:Dynamic):Int
	{
		final h=_handle(l,1); final spk=Lua.tostring(l,2); final txt=Lua.tostring(l,3);
		final col=Lua.gettop(l)>3?Std.int(Lua.tonumber(l,4)):0;
		Lua.settop(l,0); final d=_reg.get(h); if(d!=null)try d.addLine(spk,txt,col) catch(_) {}; return 0;
	}
	static function _fnDialogPortrait(l:Dynamic):Int   { final d=_reg.get(_handle(l,1)); if(d!=null)try d.setPortrait(Lua.tostring(l,2),Lua.tostring(l,3)) catch(_) {}; Lua.settop(l,0); return 0; }
	static function _fnDialogTypeSpeed(l:Dynamic):Int  { final d=_reg.get(_handle(l,1)); if(d!=null)try d.typeSpeed=Lua.tonumber(l,2) catch(_) {}; Lua.settop(l,0); return 0; }
	static function _fnDialogAutoAdv(l:Dynamic):Int    { final d=_reg.get(_handle(l,1)); if(d!=null)try d.autoAdvance=Lua.tonumber(l,2) catch(_) {}; Lua.settop(l,0); return 0; }
	static function _fnDialogSpColor(l:Dynamic):Int    { final d=_reg.get(_handle(l,1)); if(d!=null)try d.speakerColor=Std.int(Lua.tonumber(l,2)) catch(_) {}; Lua.settop(l,0); return 0; }
	static function _fnDialogBgColor(l:Dynamic):Int    { final d=_reg.get(_handle(l,1)); if(d!=null)try d.bgColor=Std.int(Lua.tonumber(l,2)) catch(_) {}; Lua.settop(l,0); return 0; }
	static function _fnDialogAllowSkip(l:Dynamic):Int  { final d=_reg.get(_handle(l,1)); if(d!=null)try d.allowSkip=Lua.toboolean(l,2) catch(_) {}; Lua.settop(l,0); return 0; }
	static function _fnDialogOnFinish(l:Dynamic):Int
	{
		final d=_reg.get(_handle(l,1)); final cb=Lua.tostring(l,2); Lua.settop(l,0);
		final luaRef=_sCurrentLua; if(d!=null)try d.onFinish=function(){ Lua.getglobal(luaRef,cb); if(Lua.type(luaRef,-1)==6) Lua.pcall(luaRef,0,0,0) else Lua.pop(luaRef,1); } catch(_) {}; return 0;
	}
	static function _fnDialogOnLine(l:Dynamic):Int
	{
		// ScriptDialog has no onLine callback — no-op (lines advance automatically)
		Lua.settop(l,0); return 0;
	}
	static function _fnDialogShow(l:Dynamic):Int   { final d=_reg.get(_handle(l,1)); Lua.settop(l,0); if(d!=null)try d.show() catch(_) {}; return 0; }
	static function _fnDialogClose(l:Dynamic):Int  { final d=_reg.get(_handle(l,1)); Lua.settop(l,0); if(d!=null)try d.close() catch(_) {}; return 0; }
	static function _fnDialogSkipAll(l:Dynamic):Int{ final d=_reg.get(_handle(l,1)); Lua.settop(l,0); if(d!=null)try d.skipAll() catch(_) {}; return 0; }
	static function _fnDialogQuick(l:Dynamic):Int
	{
		final spk=Lua.tostring(l,1); final txt=Lua.tostring(l,2); final cb=Lua.gettop(l)>2?Lua.tostring(l,3):null;
		Lua.settop(l,0);
		try { final luaRef=_sCurrentLua;
		      final d=funkin.scripting.ScriptDialog.quick(spk??'', txt??'', 0.0,
		            cb!=null ? function(){ Lua.getglobal(luaRef,cb); if(Lua.type(luaRef,-1)==6) Lua.pcall(luaRef,0,0,0) else Lua.pop(luaRef,1); } : null);
		      _pushOOP(l,d); } catch(_) { Lua.pushnil(l); }; return 1;
	}
	static function _fnDialogSequence(l:Dynamic):Int
	{
		final h=_handle(l,1); Lua.settop(l,0); final d=_reg.get(h);
		if(d!=null)try d.show() catch(_) {}; return 0;
	}
	static function _fnCloseAllDialogs(l:Dynamic):Int { Lua.settop(l,0); return 0; /* no global closeAll in this engine */ }

	// Persistent data
	static function _fnDataSave(l:Dynamic):Int
	{
		final k=Lua.tostring(l,1); final v=_readOOP(l,2); Lua.settop(l,0);
		try { flixel.FlxG.save.data.ruleScriptData ??= {}; Reflect.setField(flixel.FlxG.save.data.ruleScriptData,k,v); flixel.FlxG.save.flush(); } catch(_) {}; return 0;
	}
	static function _fnDataLoad(l:Dynamic):Int
	{
		final k=Lua.tostring(l,1); Lua.settop(l,0);
		try _pushOOP(l,Reflect.field(flixel.FlxG.save.data.ruleScriptData,k)) catch(_) Lua.pushnil(l); return 1;
	}
	static function _fnDataExists(l:Dynamic):Int
	{
		final k=Lua.tostring(l,1); Lua.settop(l,0);
		try Lua.pushboolean(l,Reflect.hasField(flixel.FlxG.save.data.ruleScriptData,k)) catch(_) Lua.pushboolean(l,false); return 1;
	}
	static function _fnDataDelete(l:Dynamic):Int
	{
		final k=Lua.tostring(l,1); Lua.settop(l,0);
		try { Reflect.deleteField(flixel.FlxG.save.data.ruleScriptData,k); flixel.FlxG.save.flush(); } catch(_) {}; return 0;
	}

	// JSON
	static function _fnJsonEnc(l:Dynamic):Int  { final v=_readOOP(l,1); Lua.settop(l,0); try Lua.pushstring(l,haxe.Json.stringify(v)) catch(_) Lua.pushnil(l); return 1; }
	static function _fnJsonDec(l:Dynamic):Int  { final s=Lua.tostring(l,1); Lua.settop(l,0); try _pushOOP(l,haxe.Json.parse(s)) catch(_) Lua.pushnil(l); return 1; }
	...
	{
		if(Lua.type(l,1)==5){
			var n=0; Lua.pushnil(l); while(Lua.next(l,-2)!=0){n++;Lua.pop(l,1);} Lua.settop(l,0); Lua.pushnumber(l,n);
		} else { Lua.settop(l,0); Lua.pushnumber(l,0); }
		return 1;
	}
	...
	{
		final txt=Lua.tostring(l,1); final dur=Lua.tonumber(l,2);
		var opts:Dynamic=null; if(Lua.gettop(l)>2&&Lua.type(l,3)==5) opts=_luaTableToOpts(l,3); Lua.settop(l,0);
		try funkin.ui.SubtitleManager.instance.show(txt,dur,opts) catch(_) {}; return 0;
	}
	static function _fnSubHide(l:Dynamic):Int    { final v=Lua.gettop(l)>0&&Lua.toboolean(l,1); Lua.settop(l,0); try funkin.ui.SubtitleManager.instance.hide(v) catch(_) {}; return 0; }
	static function _fnSubClear(l:Dynamic):Int   { Lua.settop(l,0); try funkin.ui.SubtitleManager.instance.clear() catch(_) {}; return 0; }
	static function _fnSubQueue(l:Dynamic):Int   { Lua.settop(l,0); return 0; }
	static function _fnSubStyle(l:Dynamic):Int   { var opts:Dynamic=null; if(Lua.gettop(l)>0&&Lua.type(l,1)==5) opts=_luaTableToOpts(l,1); Lua.settop(l,0); try funkin.ui.SubtitleManager.instance.setStyle(opts) catch(_) {}; return 0; }
	static function _fnSubReset(l:Dynamic):Int   { Lua.settop(l,0); try funkin.ui.SubtitleManager.instance.resetStyle() catch(_) {}; return 0; }

	// Transitions
	static function _fnFadeIn(l:Dynamic):Int
	{
		final dur=Lua.gettop(l)>0?Lua.tonumber(l,1):1.0; Lua.settop(l,0);
		try flixel.FlxG.camera.fade(flixel.util.FlxColor.BLACK, dur, true) catch(_) {}; return 0;
	}
	static function _fnFadeOut(l:Dynamic):Int
	{
		final dur=Lua.gettop(l)>0?Lua.tonumber(l,1):1.0; Lua.settop(l,0);
		try flixel.FlxG.camera.fade(flixel.util.FlxColor.BLACK, dur, false) catch(_) {}; return 0;
	}

	// State handles — direct OOP access to ALL engine states
	static function _fnGetMenuHandle(l:Dynamic):Int     { final name=Lua.tostring(l,1); Lua.settop(l,0); _pushOOP(l, flixel.FlxG.state); return 1; }
	static function _fnGetPauseHandle(l:Dynamic):Int    { Lua.settop(l,0); _pushOOP(l, try flixel.FlxG.state.subState catch(_) null); return 1; }
	static function _fnGetGameOverHandle(l:Dynamic):Int { Lua.settop(l,0); _pushOOP(l, flixel.FlxG.state); return 1; }
	static function _fnGetOptionsHandle(l:Dynamic):Int  { Lua.settop(l,0); _pushOOP(l, flixel.FlxG.state); return 1; }
	static function _fnGetResultsHandle(l:Dynamic):Int  { Lua.settop(l,0); _pushOOP(l, flixel.FlxG.state); return 1; }
	static function _fnGetFreeplayHandle(l:Dynamic):Int { Lua.settop(l,0); _pushOOP(l, flixel.FlxG.state); return 1; }
	static function _fnGetMainMenuHandle(l:Dynamic):Int { Lua.settop(l,0); _pushOOP(l, flixel.FlxG.state); return 1; }
